...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mml="http://www.w3.org/1998/Math/MathML">
	...
	<xsl:template match="ref" mode="position">{<xsl:value-of select="@id"/>}<xsl:value-of select="position()"/>{/<xsl:value-of select="@id"/>}</xsl:template>
    <xsl:template match="ref" mode="get_position"><xsl:variable name="id" select="@id"/><xsl:value-of select="substring-after(substring-before($refpos,concat('{/',$id,'}')),concat('{',$id,'}'))"/></xsl:template>
	...
	<xsl:template match="ref"><p class="ref">
            ...
            <xsl:variable name="aref">000000<xsl:apply-templates select="." mode="get_position"/></xsl:variable>
	...
	<xsl:template match="source" mode="element-content">
		...
		<xsl:apply-templates/>
			...
			<xsl:when test="../@publication-type='confproc'">
					...
					<xsl:when test="../conf-loc or ../conf-date">;</xsl:when>
				</xsl:choose>
					</xsl:when>
			<xsl:when test="../@publication-type='journal'">
						<xsl:choose>
							<xsl:when test="../edition or ../trans-source"/>
														<xsl:otherwise>.</xsl:otherwise>
				</xsl:choose>
					</xsl:when>
