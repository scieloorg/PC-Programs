...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:if
         ...
         <xsl:if test="$dtd-version = '2'">
            <xsl:if test="not(sec[@sec-type = 'display-objects'])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">display objects
                     test</xsl:with-param>
                  <xsl:with-param name="description">
      ...
      <xsl:if test="$dtd-version = '3' or $dtd-version = 'j1'">
         <xsl:if
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">floating objects
                        test</xsl:with-param>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="ms-footnote-license-check">
      ...
      <xsl:choose>
         ...
         <xsl:when
            ...
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">license in footnote
                  test</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="ms-front-content-test">
      <xsl:if
         test="notes[not(@notes-type = 'related-article') and not(descendant::related-article)]">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="abstract-sec-test">
      <xsl:if test="sec and sec/sec">
         <xsl:if test="count(sec) = 1 and count(sec/sec) > 1">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  >abstract-sec-test</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="alternatives-child-count">
      ...
      <xsl:if test="count($context/*) &lt; 2">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="concat(local-name($context), ' error ')"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="article-type-content-check">
      ...
      <xsl:variable name="normalized"
         select="normalize-space($context/@article-type)"/>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">article-type attribute
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="attribute-present-not-empty">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="
               (local-name() = 'sub-article' and not(attribute::article-type)) or
               (local-name() = 'response' and not(attribute::response-type))">
            <xsl:call-template name="make-error">
         ...
         <xsl:when
            test="
               (local-name() = 'sub-article' and not(normalize-space(@article-type))) or
               (local-name() = 'response' and not(normalize-space(@response-type)))">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type" select="$test-name"/>
               <xsl:with-param name="description">
  ...
  -->
   ...
   <xsl:template name="contract-num-linking">
      ...
      <xsl:choose>
         ...
         <xsl:when test="not(@rid[normalize-space()])">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">contract-element
                  checking</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="contrib-attribute-checking">
      ...
      <xsl:if
         test="$context/@equal-contrib = 'yes' and id($context/xref/@rid)/@fn-type[. = 'equal']">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
   ...
   <xsl:template name="contrib-author-notes-test">
      ...
      <xsl:if
         test="@deceased = 'yes' and id(xref/@rid)/@fn-type[. = 'deceased']">
         <xsl:call-template name="make-error">
      ...
      <xsl:if
         test="@equal-contrib = 'yes' and id(xref/@rid)/@fn-type[. = 'equal']">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'author notes check'"/>
            <xsl:with-param name="description">
               <xsl:text>&lt;contrib&gt; should not have an equal-contrib attribute set to 'yes' and a fn where the fn-type is 'equal'</xsl:text>
   ...
   <xsl:template name="epub-date-check">
      ...
      <xsl:if test="$context/day and not($context/month)">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'date check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mathml-repeated-element-check">
      ...
      <xsl:choose>
         ...
         <xsl:when test="name(following-sibling::node()[1]) = ''">
            <xsl:if
               test="
                  name($context) = name(following-sibling::node()[2])
         ...
         <xsl:otherwise>
            <xsl:if
               test="
                  name($context) = name(following-sibling::node()[1])
                  and not(parent::mml:msubsup)
                  and not(parent::mml:msub) and not(parent::mml:msup)
                  and not(parent::mml:mfrac) and not(parent::mml:munderover)
                  and not(parent::mml:mroot) and not(parent::mml:mfenced[@separators])">
      ...
      <xsl:choose>
         <xsl:when test="@ext-link-type = 'doi'">
            <xsl:if test="@xlink:href = $doi">
               <xsl:choose>
                  <xsl:when
                     test="@related-article-type = 'peer-reviewed-article'"/>
                  <xsl:otherwise>
                     <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type"
                           select="'related-article check'"/>
                        <xsl:with-param name="class">
                           <xsl:choose>
                              <xsl:when
                                 test="$arttype = 'letter' or $arttype = 'reply' or @related-article-type = 'alt-language'">
                                 <xsl:text>warning</xsl:text>
                              </xsl:when>
                              <xsl:otherwise>
                                 <xsl:text>error</xsl:text>
         ...
         <xsl:choose>
            ...
            <xsl:when
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type">sec-type
                           checking</xsl:with-param>
                        <xsl:with-param name="description">
   ...
   <xsl:template name="ms-extended-data-sec-test">
      <xsl:if test="@sec-type = 'extended-data'">
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">section title
                     checking</xsl:with-param>
                  <xsl:with-param name="description">
            ...
            <xsl:choose>
               <xsl:when
                  ...
                  <xsl:call-template name="make-error">
                     ...
                     <xsl:with-param name="error-type">string-date
                        checking</xsl:with-param>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="string-name-content-check">
      <xsl:if test="count(surname) > 1 or count(given-names) > 1">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">string-name
               checking</xsl:with-param>
            <xsl:with-param name="description">
