...
<xsl:transform version="1.0" id="ViewNLM-v2-04_scielo.xsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:util="http://dtd.nlm.nih.gov/xsl/util" xmlns:mml="http://www.w3.org/1998/Math/MathML" exclude-result-prefixes="util xsl">
	...
	<xsl:template match="*[@citation-type]/text()" mode="isunmarked">
		<xsl:if test="not(contains(',.; :()):',normalize-space(.)))"><xsl:value-of select="normalize-space(.)"/></xsl:if>
	</xsl:template>
	
	<xsl:template match="*[@citation-type]" mode="check-unmarked-text">
		<xsl:variable name="unmktext"><xsl:apply-templates select="text()" mode="isunmarked"/></xsl:variable>
		<xsl:comment>unmktext: <xsl:value-of select="$unmktext"/></xsl:comment>
		<xsl:if test="normalize-space($unmktext)!='' or italic">
