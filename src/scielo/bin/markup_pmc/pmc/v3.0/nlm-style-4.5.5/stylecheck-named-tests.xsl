...
<!-- style or stream-specific checks -->
	...
	<xsl:template name="ms-article-meta-abstract-test">
		<xsl:if test="count(abstract) &gt; 1">
			<xsl:call-template name="make-error">
				...
				<xsl:with-param name="description">
					<xsl:text>Article cannot have more than one &lt;abstract&gt;</xsl:text>
					</xsl:with-param>
				<xsl:with-param name="tg-target" select="'tags.html#el-abs'"/>
				</xsl:call-template>
      	</xsl:if>
   ...
   <xsl:template name="ms-article-meta-content-test">
      <xsl:if test="volume | volume-id | issue |
   	...
   	<xsl:if test="related-article and not(/article/@article-type='correction') and not(/article/@article-type='retraction') and not(/article/@article-type='article-commentary')">
   		<xsl:call-template name="make-error">
   			<xsl:with-param name="error-type">article-meta content check</xsl:with-param>
   			<xsl:with-param name="description">
   				<xsl:text>Only corrections, retractions, and article-commentary may contain &lt;related-article&gt;.</xsl:text>
   			</xsl:with-param>
   			<xsl:with-param name="tg-target" select="'tags.html#el-artmeta'"/>
   		</xsl:call-template>
   	</xsl:if>
   ...
   <!-- Template: ms-back-display-object-test
        ...
        If article is v 3.x and has floating tables/figs, then
   ...
   <xsl:template name="ms-contrib-content-test">
      ...
      <xsl:if test="address or author-comment or bio or uri">
         <xsl:call-template name="make-error">
				<xsl:with-param name="error-type" select="'contrib content check'"/>
            <xsl:with-param name="description">
               <xsl:text>&lt;contrib&gt; should not contain any of the following elements: &lt;address&gt;, &lt;author-comment&gt;, &lt;bio&gt;, &lt;uri&gt;</xsl:text>
            </xsl:with-param>
				 <xsl:with-param name="tg-target" select="'tags.html#el-contrib'"/>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template name="ms-front-content-test">
      <xsl:if test="notes">
         <xsl:call-template name="make-error">
				...
				<xsl:with-param name="description">
               <xsl:text>&lt;front&gt; should not contain a &lt;notes&gt; element unless it is a disclaimer. Place all notes inside &lt;author-notes&gt; or move to a &lt;fn-group&gt; in &lt;back&gt;.</xsl:text>
            	</xsl:with-param>
      ...
      <xsl:choose>
		...
		<xsl:choose>
            ...
            <xsl:call-template name="make-error">
					<xsl:with-param name="error-type">manuscript ProcessingInstruction check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>only one &lt;?origin?&gt; PI may be supplied. It must correspond to the input streams 'nihpa', 'ukpmcpa', 'capmc', 'hal', or 'hhmipa'.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
				</xsl:when>
         ...
         <xsl:when test="//processing-instruction('nihms')"/>
            ...
            <xsl:call-template name="make-error">
					<xsl:with-param name="error-type">manuscript ProcessingInstruction check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>&lt;?origin?&gt; PI must be supplied. It must correspond to the input streams 'nihpa', 'ukpmcpa', 'capmc', 'hal', or 'hhmipa' like &lt;?origin ukpmcpa?&gt;.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
				</xsl:when>
            ...
            <xsl:call-template name="make-error">
               ...
               <xsl:with-param name="description">
                  ...
                  <xsl:text>?&gt; is not an acceptable input stream. It must be one of 'nihpa', 'ukpmcpa', 'capmc', or 'hal'.</xsl:text>
               </xsl:with-param>
            	</xsl:call-template>
				</xsl:otherwise>
         </xsl:choose>
...
<!-- ######################################################################## -->
   ...
   <xsl:template name="abstract-attribute-test">
      ...
      <xsl:if test="@*">
         <xsl:call-template name="make-error">
            ...
            <xsl:with-param name="description">
               <xsl:text>Do not use abstract-type or xml:lang attributes.</xsl:text>
            </xsl:with-param>
				<xsl:with-param name="tg-target" select="'tags.html#el-abs'"/>
         </xsl:call-template>
      </xsl:if>
  ...
  <!-- ********************************************* -->
   ...
   <xsl:template name="abstract-title-test">
	...
	<!-- Template: alternatives-child-count 
		alternatives must have more than 1 child -->
	<!-- *********************************************************** -->
	<xsl:template name="alternatives-child-count">
		<xsl:if test="count(*)=1">
			<xsl:call-template name="make-error">
				<xsl:with-param name="error-type">alternatives child count</xsl:with-param>
				<xsl:with-param name="description">
					<xsl:text>&lt;alternatives&gt; in must contain more than 1 child</xsl:text>
				</xsl:with-param>
   ...
   <!-- Template: alternatives-content-check 
        ...
        alternatives content depends on parent
     -->
   <!-- *********************************************************** -->
   <xsl:template name="alternatives-content-check">
		<xsl:choose>
			...
			<xsl:when test="parent::app">
				<xsl:if test="not(media) and not(textual-form)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type">alternatives content check</xsl:with-param>
               		<xsl:with-param name="description">
                  		<xsl:text>&lt;alternatives&gt; in &lt;</xsl:text>
								<xsl:value-of select="name(parent::node())"/>
               			<xsl:text>&gt; must contain one of these elements: &lt;media&gt; or &lt;textual-form&gt;.</xsl:text>
               		</xsl:with-param>
						<xsl:with-param name="tg-target" select="'tags.html#el-alternatives'"/> 
            		</xsl:call-template>
					</xsl:if>
			...
			<xsl:when test="parent::chem-struct or
				...
				<xsl:if test="not(tex-math) and not(graphic) and not(mml:math)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type">alternatives content check</xsl:with-param>
               		<xsl:with-param name="description">
                  		<xsl:text>&lt;alternatives&gt; in &lt;</xsl:text>
								<xsl:value-of select="name(parent::node())"/>
               			<xsl:text>&gt; must contain one of these elements: &lt;tex-math&gt;, &lt;graphic&gt;, or &lt;mml:math&gt;.</xsl:text>
               		</xsl:with-param>
						<xsl:with-param name="tg-target" select="'tags.html#el-alternatives'"/> 
            		</xsl:call-template>
					</xsl:if>
  ...
  <!-- *********************************************************** -->
   ...
   <xsl:template name="article-type-to-related-article-check">
      ...
      <xsl:choose>
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">article-type attribute check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:text>Expression of Concern should have a related-article element with the related-article-type attribute set to 'object-of-concern'</xsl:text>
                     </xsl:with-param>
							<xsl:with-param name="class" select="'warning'"/>
                  </xsl:call-template>
               </xsl:otherwise>
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">article-type attribute check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:text>Corrections should have a related-article element with the related-article-type attribute set to 'corrected-article'</xsl:text>
                     </xsl:with-param>
							<xsl:with-param name="class" select="'warning'"/>
                  </xsl:call-template>
               </xsl:otherwise>
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">article-type attribute check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:text>Retractions should have a related-article element with the related-article-type attribute set to 'retracted-article'</xsl:text>
                     </xsl:with-param>
							<xsl:with-param name="class" select="'warning'"/>
                  </xsl:call-template>
               </xsl:otherwise>
         ...
         <xsl:when test="$context/@article-type[.='in-brief']">
            <xsl:choose>
               <xsl:when test="not($context//related-article)">
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     ...
                     <xsl:with-param name="description" select="concat(
                        'If an *in brief* article has a related-article ',
						' element, the related-article-type attribute should ',
						'be set to [article-reference], not ',
						$context//related-article/@related-article-type,
						'.')"/>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
         ...
         <xsl:when test="$context/@article-type[.='article-commentary']">
            ...
            <xsl:choose>
               <xsl:when test="$context//related-article
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     ...
                     <xsl:with-param name="class" select="'warning'"/>
                     <xsl:with-param name="description">
                        <xsl:text>Commentaries should have a related-article element with the related-article-type attribute set to 'commentary-article', not '</xsl:text>
						<xsl:value-of select="$context//related-article/@related-article-type"/>'.
                     </xsl:with-param>
                  </xsl:call-template>
               </xsl:otherwise>
   ...
   <!-- Template: back-element-check 
        ...
        Elements tested include:
         ...
         glossary (should not be in a sec inside body)
  ...
  <!-- *********************************************************** -->
   ...
   <xsl:template name="contrib-content-test">
      <xsl:choose>
         <xsl:when test="not(name) and not(collab)">
            <xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'contrib content check'"/>
               <xsl:with-param name="description">
                  <xsl:text>&lt;contrib&gt; must have either a &lt;name&gt; or a &lt;collab&gt;</xsl:text>
               </xsl:with-param>
				<xsl:with-param name="tg-target" select="'tags.html#el-contrib'"/>
            </xsl:call-template>
         </xsl:when>
         
         <xsl:when test="name[not(@content-type='index')] and collab">
            <xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'contrib content check'"/>
               <xsl:with-param name="description">
         ...
         <xsl:when test="count(name[not(@content-type='index') and not(contains(@content-type,'xml:lang'))] | collab) &gt; 1">
   ...
   <xsl:template name="doi-format-test">
      ...
      <xsl:choose>
         ...
         <!-- No spaces allowed 
			...
			SPACES ARE ALLOWED  - beck 8/21/2006
...
<!-- beck took this out 1/5/2009      	<xsl:when test="$context/@ext-link-type='email' and ($document-type='book' or $document-type='book-part')">
   ...
   <!-- Template: href-content-check 
        ...
        xlink:href attributes cannot be empty as this
        causes a PMC system error.
     -->
   <!-- *********************************************************** -->
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>		 
            ...
            <xsl:variable name="is-long-name-abbreviated-range">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string" select="'JAN-FEB-MARCH-APRIL-MAY-JUNE-JULY-AUG-SEP-OCT-NOV-DEC-'"/>
               </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="is-named-season-range-with-fall">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string" select="'SPRING-SUMMER-FALL-WINTER-'"/>
               </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="is-named-season-range-with-autumn">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string" select="'SPRING-SUMMER-AUTUMN-WINTER-'"/>
               </xsl:call-template>
            </xsl:variable>
   ...
   <!-- Template: journal-id-check 
        ...
        2) journal-id/@journal-id-type must be present, with values from:
           archive, aggregator, doi, index, issn, pmc, publisher-id, or nlm-ta
     -->
   <!-- *********************************************************** -->
   <xsl:template name="journal-id-check">
 	...
 	<xsl:variable name="ok-journal-id-type-values" select="concat( 
        ...
        <xsl:if test="$jidt-ok != 1">
            ...
            <xsl:with-param name="error-type">journal-meta-check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:text>journal-id-type attribute value is restricted to: 'archive', 'aggregator', 'doi', 'hwp', 'index', 'issn', 'pmc', 'publisher-id', 'nlm-ta'</xsl:text>
            </xsl:with-param>
         </xsl:call-template>
			</xsl:if>
   </xsl:template>
   ...
   <xsl:template name="journal-meta-check">
      ...
      <!-- journal-title is required  in v1 and v1; journal-title-group is required in v3-->
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="$dtd-version='2' and not(journal-title[normalize-space()])">
                    <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type">journal-meta-check</xsl:with-param>
                        <xsl:with-param name="description">
                ...
                <xsl:if test="$dtd-version='3' and not(journal-title-group[normalize-space()])">
                    <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type">journal-meta-check</xsl:with-param>
                        <xsl:with-param name="description">
                            <xsl:text>journal-title-group is required</xsl:text>
                        </xsl:with-param>
							<xsl:with-param name="tg-target" select="'tags.html#el-jmeta'"/>
                    </xsl:call-template>
                </xsl:if>
   ...
   <xsl:template name="list-item-content-check">
      ...
      <!--
      <xsl:if test="descendant::boxed-text | descendant::table-wrap
	              | descendant::table-wrap-group | descendant::fig">
      -->
      <xsl:if test="descendant::boxed-text[not(@content-type='inline')] |
           descendant::table-wrap[not(@position='anchor')] |
	   descendant::table-wrap-group | 
	   descendant::fig">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'list-item checking'"/>
            <xsl:with-param name="description">
   ...
   <!-- *********************************************************** -->
   <!-- Template: mathml-singleton-check
        
        Make sure that mathml is not used to tag only one character
        attribute -->
   <!-- *********************************************************** -->
<xsl:template name="mathml-singleton-check">
	<xsl:param name="context" select="."/> <!-- will be mml:math -->
	
	<xsl:choose>
		<xsl:when test="$context/descendant::*[
			@mathvariant='double-struck' or
			@mathvariant='bold-fraktur' or
			@mathvariant='script' or
			@mathvariant='bold-script' or
			@mathvariant='fraktur' or
			@mathvariant='sans-serif' or
			@mathvariant='bold-sans-serif' or
			@mathvariant='sans-serif-italic' or
			@mathvariant='sans-serif-bold-italic' or
			@mathvariant='monospace']"/>
		<!-- Report an error -->
		<xsl:when test="(count($context/child::node()[not(self::text())])=1
					and string-length($context/child::node()/text())=1
					and not($context/child::node()/*)
					and not(local-name($context) = 'msqrt')) 
			or
			(count($context/child::node()[not(self::text())])=1
			and count($context/child::node()/child::node()[not(self::text())])=1
			and string-length($context/child::node()/child::node()/text())=1
			and not($context/child::node()/child::node()/*)
			and not($context//*[local-name(.) = 'msqrt']))">
			<xsl:call-template name="make-error">
				<xsl:with-param name="error-type" select="'mathml content check'"/>
				<xsl:with-param name="description">
					<xsl:text>mml:math should be used for mathematical expressions, not to tag only one character.</xsl:text>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:when>
	</xsl:choose>
...
<!--         <xsl:message>
   ...
   <!-- Template: ppub-check
        ...
        No more than one pub-date of type "ppub" is allowed 
        in each article/sub-article/response
     -->
   <!-- *********************************************************** -->
   <xsl:template name="ppub-check">
      ...
      <xsl:if test="$context/@pub-type = 'ppub'">
         <xsl:if test="$context/preceding-sibling::pub-date[@pub-type = 'ppub']">
            <xsl:call-template name="make-error">
               ...
               <xsl:with-param name="description">
                  <xsl:text>article-meta should only contain one pub-date element with pub-type value of 'ppub'</xsl:text>
               </xsl:with-param>
				<xsl:with-param name="tg-target" select="'tags.html#el-pubdate'"/>
            </xsl:call-template>
         </xsl:if>
   ...
   <!-- Template: pub-date-check  
        Notes:    For Articles, there must be a pub-date of a type
		            other than nihms-submitted
		            
		            For articles, if there is a collection pub-date, the
		            epub-date must have both day and month
     -->
   <!-- *********************************************************** -->
   <xsl:template name="pub-date-check">
		<xsl:choose>
		<!--	<xsl:when test="pub-date[@pub-type='nihms-submitted'] and pub-date[@pub-type='pmc-release']">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type">pub-date check</xsl:with-param>
					<xsl:with-param name="description">
						<xsl:text>BOO HOO BOO HOO article-meta must contain a real publication date.</xsl:text>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:when>  -->
			<xsl:when test="pub-date[@pub-type='collection']">
				<xsl:choose>
					...
					<xsl:when test="not(pub-date[@pub-type='epub'])">
						<xsl:call-template name="make-error">
							<xsl:with-param name="error-type">pub-date check</xsl:with-param>
							<xsl:with-param name="description">
 		...
 		<xsl:if test="pubdate/day and not(pubdate/month)">
           <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
			      select="'date check'"/>
               <xsl:with-param name="description">
   ...
   <xsl:template name="pub-id-check">
	  ...
	  <xsl:variable name="ok-names" select="concat(
	     ' art-access-id coden doaj doi medline manuscript rrn ',
		 ' other pii pmcid pmid publisher-id sici ',
		 ' ')"/>
   ...
   <xsl:template name="pub-id-type-check">
	  ...
	  <!-- <xsl:variable name="ok-names" select="concat(
	     ' archive art-access-id aggregator coden doaj doi index manuscript ',
		 ' manuscript medline pii pmc pmcid pmid publisher-id sici other ',
		 ' ')"/>  -->
		...
		<xsl:variable name="ok-names">
	  		<xsl:choose>
				...
				<xsl:when test="$stream='rrn'">
					<xsl:value-of select="'knolid other publisher-id doi art-access-id'"/>
					</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="' archive art-access-id aggregator coden doaj doi index manuscript medline pii pmc pmcid pmid publisher-id sici other '"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
		<xsl:variable name="is-ok">
		...
		<xsl:if test="@pub-id-type = 'manuscript' and $style='article'">
		<xsl:variable name="props">
			<xsl:for-each select="//processing-instruction('properties')">
				<xsl:value-of select="."/>
				</xsl:for-each>
			</xsl:variable>
			<xsl:choose>
				<xsl:when test="contains($props, 'manuscript')"/>
				<xsl:when test="contains(//processing-instruction('origin'), 'pubmed')"/>
				<xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">article-id check</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:text>attribute pub-type='manuscript' should not be set for article content. Use the value 'publisher-id' to identify article-ids defined by the publisher</xsl:text>
                  </xsl:with-param>
               </xsl:call-template>	
				</xsl:otherwise>
				</xsl:choose>
   ...
   <!-- Template: pub-type-check
        ...
        Examines issn and pub-date elements for valid
        pub-type attributes. Specifically:
   ...
   <xsl:template name="pub-type-check">
      <xsl:param name="context" select="."/> <!-- issn or pub-date -->
      ...
      <xsl:choose>
         ...
         <xsl:when test="$context[self::pub-date][@pub-type ='collection']">
            <xsl:if test="not(//pub-date[@pub-type = 'epub']) and not(/article[@article-type='correction'])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
				      select="'pub-type attribute check'"/>
                  <xsl:with-param name="description" select="concat(
                     'pub-type attribute can be set to [collection] only ',
         ...
         <!-- ISSN element, if @pub-type, must be epub or ppub. Example: @pub-type='epub-ppub' -->
      	<xsl:when test="$context[self::issn]">
      		<xsl:if test="@pub-type and not(@pub-type='ppub') and not(@pub-type='epub')">
      			<xsl:call-template name="make-error">
      				<xsl:with-param name="error-type"
      					select="'pub-type attribute check'"/>
      				<xsl:with-param name="description" select="concat(
      					'@pub-type may only be set to ppub or epub',
      					' on element issn.')"/>
      				<xsl:with-param name="tg-target" select="'tags.html#el-pubdate'"/>
      			</xsl:call-template>
      		</xsl:if>
   ...
   <xsl:template name="ms-related-article-check">
   	...
   	<xsl:if test="@related-article-type!='corrected-article' and @related-article-type!='retracted-article' and @related-article-type!='commentary-article'">
   		<xsl:call-template name="make-error">
   			<xsl:with-param name="error-type">related-article type check</xsl:with-param>
   			<xsl:with-param name="description">
   				<xsl:text>&lt;related-article&gt; must contain an @related-article-type with the value "commentary-article", "corrected-article", or "retracted-article", not </xsl:text>
   	...
   	<xsl:if test="not(@ext-link-type='pmcid')">
   		<xsl:call-template name="make-error">
   			<xsl:with-param name="error-type">related-article @ext-link-type check</xsl:with-param>
   			<xsl:with-param name="description">
   				<xsl:text>&lt;related-article&gt; must contain an @ext-link-type with the value "pmcid" and the @xlink:href must include a PMC ID.</xsl:text>
   			</xsl:with-param>
   	   		</xsl:call-template>
   	</xsl:if>
   	...
   	<xsl:if test="not(number(@xlink:href))">
   		<xsl:call-template name="make-error">
   			...
   			<xsl:with-param name="description">
   				<xsl:text>&lt;related-article&gt; must contain an @xlink:href that contains a PMC ID.</xsl:text>
   			</xsl:with-param>
   	   		</xsl:call-template>
   	</xsl:if>
   ...
   <xsl:template name="related-object-check">
		...
		<xsl:choose>
			<xsl:when test="@object-id">
				<xsl:if test="not(@object-type)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type" select="'related-object check'"/>
						<xsl:with-param name="description">
							<xsl:text>The object that is the target of this &lt;related-object&gt; must be described with an @object-type.</xsl:text>
        	   	    </xsl:with-param>
						<xsl:with-param name="tg-target" select="'tags.html#el-relobj'"/>
         		   </xsl:call-template>
					</xsl:if>
			...
			<xsl:when test="@document-id ">
				<xsl:if test="not(@document-type)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type" select="'related-object check'"/>
						<xsl:with-param name="description">
							<xsl:text>The document that is the target of this &lt;related-object&gt; must be described with an @document-type.</xsl:text>
        	   	    </xsl:with-param>
						<xsl:with-param name="tg-target" select="'tags.html#el-relobj'"/>
         		   </xsl:call-template>
					</xsl:if>
			...
			<xsl:when test="@source-id ">
				<xsl:if test="not(@source-type)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type" select="'related-object check'"/>
						<xsl:with-param name="description">
							<xsl:text>The work that is the target of this &lt;related-object&gt; must be described with an @source-type.</xsl:text>
        	   	    </xsl:with-param>
						<xsl:with-param name="tg-target" select="'tags.html#el-relobj'"/>
         		   </xsl:call-template>
					</xsl:if>
...
<!-- end related-object -->
   ...
   <xsl:template name="related-article-to-article-type-attribute">
	  ...
	  <xsl:variable name="msg_start" select="concat(
         'Either the related-article-type attribute in this element or ',
		 'the article-type attribute in &lt;article&gt; is incorrect. ',
		 'The related-article-type suggests that this article ')"/>
            
      <xsl:choose>
         <!-- Stop if there is no article-type attribute -->
         <xsl:when test="string-length($art-type) = 0"/>

         <xsl:when test="$context[@related-article-type = 'addended-article']">
            <xsl:choose>
               <!-- This is correct -->
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">related-article-type check
					 </xsl:with-param>
                     <xsl:with-param name="description" select="concat(
					    $msg_start,
						' addends the related-article. ',
         ...
         <xsl:when test="$context[@related-article-type = 'commentary-article']">
            <xsl:choose>
               <!-- This is correct -->
               <xsl:when test="                                        
                  $art-type = 'editorial'                                        
				...
				<!--	<xsl:when test="$art-type 'retraction' and 
					(preceding-sibling::related-article[@related-article-type = 'retracted-article'] or
					 following-sibling::related-article[@related-article-type = 'retracted-article'])"/> -->
               <!-- Report problem -->
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type" select="'related-article-type check'"/>
                     <xsl:with-param name="description" select="concat(
					    $msg_start,
						' comments on the related-article. ',
						'However, the article-type attribute is [', $art-type,
         ...
         <xsl:when test="$context[@related-article-type = 'corrected-article']">
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
					    select="'related-article-type check'"/>
                     <xsl:with-param name="description" select="concat(
					    $msg_start,
						' corrects the related-article. ',
						'However, the article-type attribute is [', $art-type,
         ...
         <xsl:when test="$context[@related-article-type = 'retracted-article']">
            <xsl:choose>
               <!-- This is correct -->
   ...
   <!-- Template: subj-group-check 
        ...
        1) subj-group[@subj-group-type='heading']
           can only have a single subject child 
           
        2) Warning if have more than one subj-group[@subj-group-type = 'heading'] -->
   <!-- *********************************************************** -->
   ...
   <xsl:template name="tex-math-id-check">
      ...
      <xsl:if test="not($context/@id)">
         <xsl:call-template name="make-error">
            ...
            <xsl:with-param name="description">
               <xsl:text>tex-math elements must have an id attribute</xsl:text>
            </xsl:with-param>
         </xsl:call-template>
      </xsl:if>
   </xsl:template> 
		...
		<xsl:when test="not(id($context/@rid))">
	     ...
	     <xsl:when test="$context/@ref-type='app'">
	        <xsl:if test="not(id($context/@rid)
	           [self::app or self::app-group or self::sec/ancestor::app or self::sec/parent::back or self::book-part[@book-part-type='appendix']])">
	           <xsl:call-template name="make-error">   
	              <xsl:with-param name="error-type">xref checking</xsl:with-param>
	              <xsl:with-param name="description">
	                 <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='author-notes' or
            ...
            <xsl:if test="not(id($context/@rid)
			   [self::author-notes or self::fn/parent::author-notes or
			    self::corresp/parent::author-notes or 
			    parent::boxed-text[@content-type='author-notes']])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='corresp'">
            <xsl:if test="not(id($context/@rid)
			  [self::corresp or self::fn or self::author-notes or 
			    parent::boxed-text[@content-type='author-notes']])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:when test="$context/@ref-type='disp-formula'">
            <xsl:if test="not(id($context/@rid)[self::disp-formula or self::disp-formula-group or self::fig or self::supplementary-material or self::inline-formula])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='list'">
            <xsl:if test="not(id($context/@rid)
			   [self::list or self::list-item or
			    self::def-list or self::def-item or
			    self::glossary or self::gloss-group])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='sec'">
            <xsl:if test="not(id($context/@rid)
			   [self::sec or self::ack or self::app or
			    self::boxed-text or self::notes or self::book-part or self::ref-list])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='table'">
            <xsl:if test="not(id($context/@rid)[self::table-wrap]) and
			              not(id($context/@rid)[self::table-wrap-group])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
         ...
         <xsl:when test="$context/@ref-type='table-fn'">
            <xsl:if test="not(id($context/@rid)[(self::fn and ancestor::table-wrap) or self::td])">
               <xsl:call-template name="make-error">   
                  <xsl:with-param name="error-type">xref checking</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
