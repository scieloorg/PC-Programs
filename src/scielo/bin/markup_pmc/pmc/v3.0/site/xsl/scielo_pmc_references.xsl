...
<xsl:transform version="1.0" id="ViewNLM-v2-04_scielo.xsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:util="http://dtd.nlm.nih.gov/xsl/util" xmlns:mml="http://www.w3.org/1998/Math/MathML" exclude-result-prefixes="util xsl">
	...
	<xsl:template match="ref">
		<p id="{@id}">
			<span id="x{@id}">
				...
				<xsl:apply-templates select="mixed-citation|citation|nlm-citation"/>
			</span>
			<!--xsl:apply-templates select="citation/pub-id|nlm-citation/pub-id" mode="link"/-->
			<xsl:apply-templates select="element-citation|citation|nlm-citation" mode="link">
	...
	<xsl:template match="ref[mixed-citation]">
		<xsl:comment>_references.xsl</xsl:comment>
		<p id="{@id}">
			<xsl:apply-templates select="mixed-citation"/>
			<!--xsl:apply-templates select="citation/pub-id|nlm-citation/pub-id" mode="link"/-->
			<xsl:apply-templates select="element-citation" mode="link">
				<xsl:with-param name="pos" select="position()"/>
				<xsl:with-param name="pub-id" select=".//pub-id"/>
			</xsl:apply-templates>
			<xsl:call-template name="nl-1"/>
	...
	<xsl:template match="element-citation|citation|nlm-citation" mode="link">
		<xsl:param name="pos"/>
