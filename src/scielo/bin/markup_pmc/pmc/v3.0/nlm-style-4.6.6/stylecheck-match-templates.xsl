...
<!-- ######################################################################## -->
   ...
   <!-- Match: alternatives
         ...
         2) must have more than 1 child
         3) content is based on parent
     -->
  ...
  <!-- ********************************************************************** -->
   ...
   <xsl:template match="mml:math">
      ...
      <xsl:call-template name="mathml-singleton-check"/>
...
<!--         <xsl:message>
   ...
   <!-- Match: pub-date
        1) cannot be empty 
        2) only one pub-date of type ppub is allowed
        3) restrict pub-type attribute values
		  4) article may have only one epub date
     -->
   <!-- *********************************************************** -->
   <xsl:template match="pub-date">
      <xsl:call-template name="empty-element-check"/>
      <xsl:call-template name="ppub-check"/>
      <xsl:call-template name="pub-type-check"/>
		<xsl:call-template name="epub-date-check">		  
			<xsl:with-param name="context" select="."/>
			</xsl:call-template>
      <xsl:apply-templates select="." mode="output"/>
		</xsl:template>
