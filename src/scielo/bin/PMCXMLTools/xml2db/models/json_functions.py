...
#from tables.table_conversion import ConversionTables
...
class JSON_Normalizer:
    def __init__(self, conversion_tables, entities, json_values):
    ...
    def format_for_indexing(self, json_record):
        ...
        if type(json_record) == type({}):
            ...
            if type(json_record) == type(''):
                json_record_dest = self.entities.remove_accent(self.conversion_tables.remove_formatting(json_record))
            else:
                if type(json_record) == type([]):
                    a = []
    ...
    def convert_value(self, json_data, tag, table_name):
        a = self.json_values.return_singleval(json_data, tag)
        if len(a)>0:
...
class JSON_Dates:
    def __init__(self, conversion_table, json_values):
    ...
    def normalize_dates(self, json_data, tag, tag_iso, tag_not_iso):
        ...
        publication_dates = self.json_values.return_value(json_data, tag)
        print('dates')
    ...
    def normalize_citation_dates(self, json_data, tag, tag_iso, tag_not_iso):
        #print(json_data)
        day = self.json_values.return_singleval(json_data, tag + 'd')
        month = self.json_values.return_singleval(json_data, tag + 'm')
        season = self.json_values.return_singleval(json_data, tag + 's')
        year = self.json_values.return_singleval(json_data, tag )
