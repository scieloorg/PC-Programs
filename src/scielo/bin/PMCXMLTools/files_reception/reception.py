...
class Reception:
    ...
    def extract_files(self, compressed_file, download_path, extracted_path, backup_path, report_sender):   
        for compressed in os.listdir(download_path):
            ...
            compressed_file.extract_files(download_path + '/' + compressed, extracted_path + '/' + compressed)

            self.report.write('Do backup ' + backup_path, True, False, True)
            compressed_file.backup(download_path + '/' + compressed, backup_path)
            attached_files = []
