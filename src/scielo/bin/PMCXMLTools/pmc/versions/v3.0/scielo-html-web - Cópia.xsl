...
<!-- ============================================================= -->
  ...
  <xsl:template match="article" mode="text-content">
	...
	<!--
		Inserido por Gustavo Silva
		Template no fim da folha de estilos()
	-->
...
<!--  Includes mode "metadata" for front matter, along with 
  ...
  <xsl:template match="trans-subtitle" mode="metadata">
	...
	<h3 class="document-trans-subtitle">
		<xsl:apply-templates/>
	</h3>
  </xsl:template>
...
<!-- ============================================================= -->
  ...
  <xsl:template match="inline-graphic">
    <xsl:apply-templates/>
    <img alt="{@xlink:href}" class="inline-graphic">
      <xsl:for-each select="alt-text">
        <xsl:attribute name="alt">
          <xsl:value-of select="normalize-space(.)"/>
        </xsl:attribute>
...
<!-- ============================================================= -->
  ...
  <xsl:template name="assign-src">
    <xsl:for-each select="@xlink:href">
	  ...
	  <xsl:value-of select="$path_img"/><xsl:value-of select="."/>.jpg
      </xsl:attribute>
    </xsl:for-each>
  </xsl:template>
...
<!-- ============================================================= -->
	...
	<!--
	Inserido por Gustavo Silva para exibir miniaturas
	-->
  
  
  <!---
  ****************************************************************
  Um fork da preparação das citações com saída xml para saída html
  
  Arquivo orginal: xsl/pmc/v3.0/jpub/citations-prep/jpub3-PMCcit.xsl
  ****************************************************************
  -->
...
<!-- Mostly this code follows the logic of cit-render.xsl            -->
  ...
  <xsl:template match="pub-id[@pub-id-type='pmid']">
    ...
    <xsl:text>[</xsl:text>
    <a href="{concat('http://www.ncbi.nlm.nih.gov/pubmed/',string(.))}" target="_blank">
      <xsl:apply-templates/>
	</a>
    <xsl:text>]</xsl:text>
  </xsl:template>
...
<!-- =============================================================== -->
  ...
  ******************
  ...
  <xsl:apply-templates/>
    <a href="{concat($path_img,@xlink:href)}.jpg">
  ...
  <xsl:template name="itens_nav">
    <xsl:for-each select="document($xml_article)/article">
		<xsl:for-each select="abstract/title | body/*/title |
							  back/title | back[not(title)]/*/title">			  
			<li>
