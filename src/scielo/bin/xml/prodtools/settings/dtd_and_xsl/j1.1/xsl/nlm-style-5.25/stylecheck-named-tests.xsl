...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   xmlns:ali="http://www.niso.org/schemas/ali/1.0/" version="1.0">
         ...
         <xsl:if test="ancestor::abstract">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  select="'abstract sec-type check'"/>
   ...
   <xsl:template name="ms-article-meta-content-test">
      <xsl:if
         ...
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">article-meta content
      ...
      <xsl:if
         test="
            related-article[not(@related-article-type = 'republished-article') and
               not(@related-article-type = 'concurrent-pub') and 
               not(@related-article-type = 'updated-article') and
               not(@related-article-type = 'addended-article') and
               not(@related-article-type = 'reprinted-article')] and 
            not(/article/@article-type = 'correction') and
   ...
   <!-- Template: ms-article-id-test
        ...
        Must have an article-id with pub-id-type="manuscript" (unless it's a Gates manuscript)
   ...
   <xsl:template name="ms-article-id-test">
      <xsl:if test="not(article-id[@pub-id-type = 'manuscript'])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">manuscript article-id
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="ms-back-content-test">
      <xsl:if test="title">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">back content
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="ms-back-display-object-test">
      <xsl:if
         test="//fig[@position = 'float'] or //table-wrap[@position = 'float']">
         <xsl:if test="$dtd-version = '2'">
            <xsl:if test="not(sec[@sec-type = 'display-objects'])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">display objects check</xsl:with-param>
                  <xsl:with-param name="description">
      ...
      <xsl:if test="$dtd-version = '3' or $dtd-version = 'j1'">
         <xsl:if
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">floating objects check</xsl:with-param>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="ms-footnote-license-check">
      ...
      <xsl:choose>
         ...
         <!-- it is possible to have cc licenses for table material -->
         <xsl:when
            test="contains($fntext, 'creativecommons.org') or contains(descendant::node()/@xlink:href, 'creativecommons.org')">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">license in footnote check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="ms-contrib-attribute-test">
      ...
      <xsl:if
         test="@contrib-type and (@contrib-type != 'author' and @contrib-type != 'editor')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'contrib attribute'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="ms-contrib-content-test">
      ...
      <xsl:if
         test="address or author-comment or bio or uri[not(@content-type = 'orcid')]">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="ms-contrib-group-content-test">
      ...
      <xsl:if
         test="*[not(self::contrib) and not(self::aff) and not(self::on-behalf-of) and not(self::xref)]">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'contrib group content check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="ms-front-content-test">
		<xsl:variable name="notes-type-values" 
   		select="
            concat(
            'disclaimer ',
            'related-article ',
            'republication-note ')"/>
	     <xsl:variable name="has-allowed-notes-type">
         <xsl:call-template name="is-in-list">
            <xsl:with-param name="list" select="$notes-type-values"/>
            <xsl:with-param name="token" select="notes/@notes-type"/>
            <xsl:with-param name="case" select="'1'"/>
         </xsl:call-template>
      </xsl:variable>

      <xsl:if
         test="$has-allowed-notes-type != '1'">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="manuscript-pi-test">
      <xsl:choose>
         ...
         <xsl:when test="//processing-instruction('nihms')">
            ...
            <xsl:call-template name="make-error">
               <xsl:with-param name="class">warning</xsl:with-param>
               <xsl:with-param name="error-type">manuscript
                  ProcessingInstruction check</xsl:with-param>
               <xsl:with-param name="description">
         ...
         <xsl:when test="not(//processing-instruction('origin'))">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">manuscript
                  ProcessingInstruction check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>&lt;?origin?&gt; PI must be supplied. It must correspond to the input streams 'nihpa', 'ukpmcpa', 'capmc', 'hal', 'hrams', 'hhmipa', or 'dhspa' like &lt;?origin ukpmcpa?&gt;.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
         </xsl:when>
         <xsl:when test="contains(//processing-instruction('origin'), 'nihpa')"/>
         <!-- ok -->
         <xsl:when
            test="contains(//processing-instruction('origin'), 'ukpmcpa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'capmc')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'hal')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'hhmipa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'hrams')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'hhspa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'asms')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'aspa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'vapa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'nasapa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'nistpa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'ahrqpa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'epapa')"/>
         <!-- ok -->
         <xsl:when test="contains(//processing-instruction('origin'), 'dhspa')"/>
         <!-- ok -->
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">manuscript
               ...
               <xsl:with-param name="description">
                  ...
                  <xsl:text>?&gt; is not an acceptable input stream. It must be one of 'nihpa', 'hhspa', 'ukpmcpa', 'capmc', 'hrams', 'hal', 'asms', 'aspa', 'vapa', 'nistpa', 'ahrqpa', 'hhmipa', or 'dhspa'.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <xsl:template name="ms-subj-group-test">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="
               count(subj-group[@subj-group-type]) = 1
               and subj-group/@subj-group-type[normalize-space() = 'heading']
         ...
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">subject-group
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="ms-extended-data-test">
      <xsl:if
         ...
         <xsl:choose>
            <xsl:when test="child::sec[@sec-type = 'extended-data']"/>
            <!-- good -->
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">extended data
                     check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="abstract-sec-test">
      <xsl:if test="sec and sec/sec">
         <xsl:if test="count(sec) = 1 and count(sec/sec) > 1">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  >abstract-section check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="abstract-title-test">
      <xsl:if
         test="translate(normalize-space(title), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = 'abstract'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">abstract title
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="aff-alternatives-content-check">
      ...
      <!-- 2 -->
      <xsl:if
         test="not($context/aff[@xml:lang]) and not($context/aff[@specific-use])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">
            ...
            <xsl:with-param name="tg-target"
               select="'tags.html#el-aff-alternatives'"/>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template name="alternatives-content-check">
      ...
      <xsl:choose>
         <xsl:when test="parent::chem-struct">
            <xsl:if test="chem-struct">
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-alternatives'"/>
               </xsl:call-template>
            </xsl:if>
         </xsl:when>
         ...
         <xsl:when
            ...
            <!-- PMC-37080: disallow multiple math elements -->
            <xsl:if test="count(mml:math) &gt; 1">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">alternatives content
                     check</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:text>Multiple &lt;mml:math&gt; elements are not allowed in &lt;alternatives&gt;</xsl:text>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-alternatives'"/>
               </xsl:call-template>
         ...
         <xsl:when
            test="
               parent::inline-supplementary-materials or
               parent::supplementary-materials">
            <xsl:if
               test="not(textual-form) and not(graphic) and not(media) and not(table)">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">alternatives content
                     check</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:when test="parent::table-wrap">
            <xsl:if
               test="not(textual-form) and not(preformat) and not(table) and not(graphic) and not(array)">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">alternatives content
                     check</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:when test="parent::bio">
            <xsl:if
               test="not(textual-form) and not(graphic) and not(media)">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">alternatives content
                     check</xsl:with-param>
                  <xsl:with-param name="description">
                     <xsl:text>&lt;alternatives&gt; in &lt;</xsl:text>
                     <xsl:value-of select="name(parent::node())"/>
                     <xsl:text>&gt; must contain one of these elements: &lt;textual-form&gt;, &lt;graphic&gt; or &lt;media&gt;.</xsl:text>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-alternatives'"/>
               </xsl:call-template>
   ...
   <xsl:template name="alternatives-parent-check">
      <xsl:variable name="alternatives-parent-values"
         select="
            concat(
            'app ',
            'array ',
      ...
      <xsl:if test="$is-in-approved-parent != '1'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">alternatives parent
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="alternatives-unique-att-values">
      ...
      <xsl:variable name="my-att-value"
         select="$nodes[1]/@*[name(.) = $att-name]"/>
      <xsl:if test="$nodes">
         <xsl:choose>
            <xsl:when
               test="$my-att-value = $nodes[position() != 1]/@*[name(.) = $att-name]">
   ...
   <xsl:template name="alternatives-xml-lang-check">
      ...
      <xsl:if
         test="count($nodes[@xml:lang = $art-lang-att] | $nodes[not(@xml:lang)]) != 1">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">
               <xsl:value-of select="$context"/>
               <xsl:text> check</xsl:text>
   ...
   <xsl:template name="article-id-content-check">
      ...
      <xsl:choose>
         <xsl:when
            test="contains($context[normalize-space()], ' ') and @pub-id-type = 'manuscript'">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">article-id
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="article-language-check">
      ...
      <xsl:if test="$art-lang-att = 'en'">
         <xsl:choose>
            <xsl:when
               test="
                  contains($bodytext, ' the ') and
                  contains($bodytext, ' a ') and
                  contains($bodytext, ' is ')"/>
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">language
                     check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="article-pi-check">
      ...
      <xsl:if
         test="//processing-instruction('OLF') or contains(//processing-instruction('properties'), 'OLF')">
         <xsl:if
            test="contains(//processing-instruction('properties'), 'no_embargo')">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">article-pi
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="article-release-delay-check">
      ...
      <xsl:for-each select="//processing-instruction('release-delay')">
         ...
         <xsl:choose>
            ...
            <xsl:when
               test="normalize-space($months) = '' or normalize-space($days) = ''">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">release-delay-pi
                     check</xsl:with-param>
                  <xsl:with-param name="description">
                  ...
                  <xsl:with-param name="tg-target"
                     select="'genprac.html#embargo'"/>
               </xsl:call-template>
            </xsl:when>

            <xsl:when
               test="normalize-space($months) = 'O' or normalize-space($days) = 'O'">
            ...
            <xsl:when
               test="not(contains(., '|')) or (not(number($months)) and normalize-space($months) != '0') or (not(number($days)) and normalize-space($days) != '0')">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">release-delay-pi
                     check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="article-type-content-check">
      ...
      <xsl:variable name="normalized">
         <xsl:choose>
            <xsl:when test="name($context)='response'">
               <xsl:value-of select="normalize-space($context/@response-type)"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="normalize-space($context/@article-type)"/>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
      ...
      <xsl:choose>
         ...
         <!-- A response or sub-article must not contain a correction/retraction/expression-of-concern -->
         <xsl:when test="name($context)='sub-article' 
            or name($context)='response'">
            <xsl:choose>
               <xsl:when test="$normalized = 'correction' 
                  or $normalized = 'retraction' 
                  or $normalized = 'expression-of-concern'">
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">article-type attribute value check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:value-of select="name($context)"/>
                        <xsl:text> should not have </xsl:text>
                        <xsl:if test="name($context)='sub-article'">
                           <xsl:text>an article</xsl:text>
                        </xsl:if>
                        <xsl:if test="name($context)='response'">
                           <xsl:text>a response</xsl:text>
                        </xsl:if>
                        <xsl:text>-type value of '</xsl:text><xsl:value-of select="$normalized"/><xsl:text>'</xsl:text>.</xsl:with-param>
         ...
         <xsl:when
            test="//processing-instruction('properties')[contains(., 'scanned_data')] and $normalized = 'index'">
            <!-- This is correct -->
         </xsl:when>

         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">article-type attribute value
                  check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>Unknown article-type value '</xsl:text><xsl:value-of
                     select="$normalized"/>'. </xsl:with-param>
               <xsl:with-param name="tg-target" select="'dobs.html#dob-arttype'"
               />
            </xsl:call-template>
         </xsl:otherwise>
   ...
   <xsl:template name="article-type-to-product-check">
      ...
      <xsl:if
         test="$context/@article-type[. = 'book-review' or . = 'product-review']">
         <xsl:choose>
            ...
            <xsl:when
               test="
                  //processing-instruction('properties')
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">product review 
                     check</xsl:with-param>
                  <xsl:with-param name="class" select="'warning'"/>
                  <xsl:with-param name="description">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$context/@article-type[. = 'expression-of-concern']">
            ...
            <xsl:choose>
               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'object-of-concern']
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">expression-of-concern related article check</xsl:with-param>
                     <xsl:with-param name="description">
         ...
         <xsl:when test="$context/@article-type[. = 'correction']">
            ...
            <xsl:choose>
               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'corrected-article']
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">correction related object check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:text>Corrections should have a related-article element with the related-article-type attribute set to 'corrected-article' or a related-object element with the link-type attribute set to 'corrected-article'</xsl:text>
                     </xsl:with-param>
                     <xsl:with-param name="class" select="'error'"/>
                  </xsl:call-template>
               </xsl:otherwise>
         ...
         <xsl:when test="$context/@article-type[. = 'retraction']">
            ...
            <xsl:choose>
               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'retracted-article']
                     or $context//related-article[@related-article-type = 'retracted-republished-article']
                     or $context//related-object[@link-type = 'retracted-article']
                     or $context//related-object[@link-type = 'retracted-republished-article']">
                  <!-- This is correct-->
               </xsl:when>

               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">retraction related object check</xsl:with-param>
                     <xsl:with-param name="description">
         ...
         <xsl:when test="$context/@article-type[. = 'in-brief']">
            <xsl:choose>
               <xsl:when
                  test="not($context//related-article) and not($context//related-object)">
                  <!-- No related article links at all: that's ok -->
               </xsl:when>

               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'article-reference']
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">in-brief article related article check</xsl:with-param>
                     <xsl:with-param name="class" select="'warning'"/>
                     <xsl:with-param name="description">
                        <xsl:choose>
                           <xsl:when test="$context//related-object">
                              ...
                              <xsl:value-of
                                 select="$context//related-object/@link-type"/>
                           </xsl:when>
                           <xsl:otherwise>
                              ...
                              <xsl:value-of
                                 select="$context//related-article/@related-article-type"
                              />
                           </xsl:otherwise>
                        </xsl:choose>
                     </xsl:with-param>
         ...
         <xsl:when test="$context/@article-type[. = 'reply']">
            ...
            <xsl:choose>
               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'letter']
               ...
               <xsl:when
                  test="//processing-instruction('properties')[contains(., 'scanned_data')]">
                  <!-- Ignore any missing related-article element -->
               </xsl:when>

               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">letter reply related article check</xsl:with-param>
                     <xsl:with-param name="class" select="'warning'"/>
                     <xsl:with-param name="description">
                        ...
                        <xsl:value-of
                           select="$context//related-article/@related-article-type | $context//related-object/@link-type"
                        />'. </xsl:with-param>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
         ...
         <xsl:when test="$context/@article-type[. = 'article-commentary']">
            ...
            <xsl:choose>
               <xsl:when
                  test="
                     $context//related-article[@related-article-type = 'commentary-article']
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">article commentary related article check</xsl:with-param>
                     <xsl:with-param name="class" select="'warning'"/>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="back-element-check">
      ...
      <xsl:variable name="has-error">
         <xsl:choose>
            <xsl:when test="$context[self::ack]">
               <xsl:choose>
                  <xsl:when
                     test="$context[parent::sec/ancestor::body or parent::body]">
            ...
            <xsl:when
               ...
               <xsl:choose>
                  ...
                  <xsl:when
                     test="$context[parent::sec/ancestor::body] and parent != 'boxed-text'">
                     <xsl:text>true</xsl:text>
                  </xsl:when>
      ...
      <xsl:if test="$has-error = 'true'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">back element
               check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:value-of select="name($context)"/>
      ...
      <xsl:variable name="has-glossary">
         <xsl:choose>
      ...
      <!-- Output error if not correct -->
      <xsl:if test="$has-glossary = 'true'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">glossary  element
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="block-formatting-check">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               ...
               <xsl:with-param name="tg-target"
                  select="'genprac.html#formatting'"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
	...
	<!-- *********************************************************** -->
	<!-- Template: book-main-unit-check 
   
        Books: Helper template takes an element and checks whether
        it has a title and id 
        
     -->
	<!-- *********************************************************** -->
	<xsl:template name="book-main-unit-check">
		<xsl:param name="context" select="."/>
		
		<xsl:if test="
				$stream = 'book' and 
				($context/self::book-part | $context/self::book-app | parent::book-back | parent::front-matter/parent::book) and
				not(/book[not(book-back)]/book-body[count(book-part) = 1]/book-part[not(book-part-meta)])
			">
			<!-- check title -->
			<xsl:if test="
				not(   $context/title[normalize-space()]			 
					 | $context/title-group/title[normalize-space()]
					 | $context/toc-title-group/title[normalize-space()]
					 | $context/index-title-group/title[normalize-space()] 
					 | $context/book-part-meta/title-group/title[normalize-space()]
					)
				">
				
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'book main unit check'"/>
					<xsl:with-param name="class">error</xsl:with-param>
					<xsl:with-param name="description">	
						<xsl:text>&lt;</xsl:text>
						<xsl:value-of select="name($context)"/>
						<xsl:text>&gt; must have a title</xsl:text>
					</xsl:with-param>
					<xsl:with-param name="tg-target"
						select="'genprac.html#bk_struct'"/>
				</xsl:call-template>
			...
			<xsl:if test="
				not(
					  $context/@id[normalize-space()]
					| $context/parent::book-part-wrapper/@id[normalize-space()]
				)">
				...
				<xsl:call-template name="make-error">
					...
					<xsl:with-param name="description">		
						<xsl:text>&lt;</xsl:text>
						<xsl:value-of select="name($context)"/>
						<xsl:text>&gt; must have an ID</xsl:text>
					</xsl:with-param>
					<xsl:with-param name="tg-target"
						select="'genprac.html#bk_struct'"/>
				</xsl:call-template>
	...
	<!-- *********************************************************** -->
	<!-- Template: book-stream-id-test
   
        Books: Helper template to check ID presence for several elements
        
     -->
	<!-- *********************************************************** -->
	<xsl:template name="book-stream-id-test">
		<xsl:if test="$stream = 'book'">
			<xsl:choose>
				<!-- FRONT-MATTER -->
				<xsl:when test="self::ack and not(parent::back | parent::body | parent::book-app-group |  parent::book-back | parent::front-matter | parent::named-book-part-body)"/>
				<xsl:when test="self::bio and not(parent::back | parent::book-back | parent::front-matter )"/>
				<!-- dedication: all -->
				<xsl:when test="self::glossary and not(parent::back | parent::book-back | parent::front-matter )"/>
				<!-- front-matter-part: all -->
				<!-- foreword: all -->
				<!-- preface: all -->
				<xsl:when test="self::notes and not(parent::back | parent::book-back | parent::front-matter)"/>				
				<xsl:when test="self::ref-list and not(parent::back | parent::book-back | parent::front-matter)"/>
				<!-- BODY / BOOK-BODY -->
				<!-- book-part: all -->
				<xsl:when test="self::sec and not(parent::back | parent::body | parent::book-app-group  | parent::named-book-part-body)"/>
				<!-- BACK / BOOK-BACK -->
				<!-- book-app-group: all -->
				<!-- app: all -->
				<!-- app-group: all -->
				...
				<!-- boxed-text: all -->
				<!-- fig, fig-group: all -->
				<!-- table-wrap, table-wrap-group: all -->				
				<xsl:when test="(self::mixed-citation | self::element-citation) and parent::ref and count(../mixed-citation | ../element-citation) = 1"/>
				<!-- UNDOCUMENTED EXCEPTIONS -->
				<xsl:when test="@sec-type = 'figs-and-tables' or @sec-type = 'link-group' "/>
				<xsl:when test="parent::book-part-wrapper/@id"/>
				<xsl:otherwise>
					<xsl:call-template name="id-present-test"/>		
				</xsl:otherwise>
			</xsl:choose>
   ...
   <xsl:template name="caption-format-test">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:choose>
               <xsl:when
                  test="count($p-to-check[1]/node()[not(self::text()[not(normalize-space())])]) = 1 and $p-to-check[1]/node()[self::bold or self::italic or self::monospace or self::sc or self::strike or self::underline]">
                  <xsl:call-template name="make-error">
               ...
               <xsl:otherwise>
                  <xsl:call-template name="caption-format-test">
                     <xsl:with-param name="recursing" select="1"/>
                     <xsl:with-param name="p-to-check"
                        select="$p-to-check[position() != 1]"/>
                     <xsl:with-param name="tg-target"
                        select="'genprac.html#formatting'"/>
                  </xsl:call-template>
               </xsl:otherwise>
   ...
   <xsl:template name="check-abstract-content">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">abstract content
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="check-abstract-type">
      <xsl:param name="context" select="."/>
      <xsl:variable name="first-without"
         select="$context/parent::node()/abstract[not(@abstract-type)][1]"/>

      <xsl:choose>
         <!-- This is the first abstract without a type: that's ok -->
         <xsl:when
            test="
               $context[not(@abstract-type)]
         ...
         <xsl:when test="$context[not(@abstract-type)]">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">abstract type
                  check</xsl:with-param>
               <xsl:with-param name="description">abstract requires an
                  abstract-type attribute to distinguish it from the other
                  abstracts in this article</xsl:with-param>
               <xsl:with-param name="tg-target" select="'tags.html#el-abs'"/>
            </xsl:call-template>
         </xsl:when>
      </xsl:choose>
   ...
   <xsl:template name="check-for-xref">
      ...
      <xsl:if test="//xref[@rid = $ID] and $editstyle = 'word'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">object reference
               check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:text>&lt;</xsl:text>
  ...
  -->
   ...
   <xsl:template name="float-obj-check">
      ...
      <xsl:if test="$ref-rid = 'missing'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">No cross-reference to floating
               object </xsl:with-param>
            <xsl:with-param name="description">
               <xsl:value-of select="local-name($context)"/>
   ...
   <xsl:template name="citation-alternatives-content-check">
      ...
      <xsl:if test="count($context/element-citation) > 1">
         <!-- element-citation: 2 -->
         <xsl:if
            test="not($context/element-citation[@xml:lang]) and not($context/element-citation[@specific-use])">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">
               ...
               <xsl:with-param name="tg-target"
                  select="'tags.html#el-citation-alternatives'"/>
            </xsl:call-template>
         </xsl:if>
         <!-- element-citation: 3 -->
      ...
      <xsl:if test="count($context/mixed-citation) > 1">
         <!-- mixed-citation: 2 -->
         <xsl:if
            test="not($context/mixed-citation[@xml:lang]) and not($context/mixed-citation[@specific-use])">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">
                  <xsl:text>citation-alternatives check</xsl:text>
               </xsl:with-param>
   ...
   <xsl:template name="collab-alternatives-content-check">
      ...
      <!-- 2 -->
      <xsl:if
         test="not($context/collab[@xml:lang]) and not($context/collab[@specific-use])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">
            ...
            <xsl:with-param name="tg-target"
               select="'tags.html#el-collab-alternatives'"/>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template name="collab-content-check">
      <xsl:param name="context" select="."/>
      <xsl:if
         test="normalize-space($context/text()) = '' and not($context/*[not(self::contrib-group)])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">collab check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:text>&lt;</xsl:text>
               <xsl:value-of select="name($context)"/>
   ...
   <xsl:template name="collab-contribgrp-check">
      ...
      <xsl:if test="$context/contrib/collab">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">contributor check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:text>&lt;</xsl:text>
               <xsl:value-of select="name($context)"/>
   ...
   <xsl:template name="contract-num-linking">
      ...
      <xsl:choose>
         ...
         <xsl:when test="not(@id[normalize-space()])">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">contract-element check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="contract-sponsor-linking">
      ...
      <xsl:variable name="contract-sponsor-ref-check">
         <xsl:for-each select="id(@rid)">
            <xsl:choose>
               <xsl:when
                  test="self::contract-sponsor and contains($context/@rid, @id)"/>
               <xsl:otherwise>
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="
               (not(@id[normalize-space()]) and //contract-num)
         ...
         <xsl:when
            test="$bad-targets or contains($contract-sponsor-ref-check, 'badstyle')">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">contract element check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="contrib-content-test">
      <xsl:choose>
         ...
         <xsl:when test="name and name-alternatives">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  select="'contrib content check'"/>
               <xsl:with-param name="description">
                  <xsl:text>&lt;contrib&gt; cannot contain a &lt;name&gt; and &lt;name-alternatives&gt;</xsl:text>
               </xsl:with-param>
               <xsl:with-param name="tg-target" select="'tags.html#el-contrib'"
               />
            </xsl:call-template>
         ...
         <xsl:when
            test="$dtd-version != 'j1' and count(name[not(@content-type = 'index') and not(contains(@content-type, 'xml:lang'))]) > 1">
         ...
         <xsl:when
            test="$dtd-version = 'j1' and (count(name) > 1 or count(collab) > 1)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  select="'contrib content check'"/>
               <xsl:with-param name="description">
   ...
   <xsl:template name="contrib-id-check">
      ...
      <xsl:if test="not($context/@contrib-id-type)">
         <xsl:call-template name="make-error">
            ...
            <xsl:with-param name="tg-target" select="'tags.html#el-contrib-id'"/>
         </xsl:call-template>
      </xsl:if>
         ...
         <xsl:choose>
            <xsl:when
               test="
                  ...
                  starts-with(., 'https://orcid.org') or
                  starts-with(., 'ORCID.org') or
                  starts-with(., 'http://ORCID.org') or
                  starts-with(., 'https://ORCID.org')">
               <!-- this is okay, output context -->
            </xsl:when>
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     select="'ORCID contrib-id check'"/>
                  <xsl:with-param name="description">
                     <xsl:text>ORCID values must start with 'http://orcid.org/' or 'https://orcid.org/'</xsl:text>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-contrib-id'"/>
               </xsl:call-template>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:if>
   ...
   <!-- *********************************************************** -->
   <!-- Template: orcid-in-uri-check
		 1) xlink:href must conform to ORCID format
		-->
   <!-- *********************************************************** -->
   <xsl:template name="orcid-in-uri-check">
       <xsl:choose>
         <xsl:when
            test="
            starts-with(., 'orcid.org') or
   ...
   <!-- Template: copyright-year-check
     ...
     -->
   <!-- *********************************************************** -->
   <xsl:template name="copyright-statement-check">
      <xsl:param name="context" select="."/>
      <xsl:if
         test="contains($context, 'Creative Commons') or contains($context, 'license')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'copyright-statement check'"/>
            <xsl:with-param name="description">
               <xsl:text>copyright-statement should not contain license information</xsl:text>
            </xsl:with-param>
            <xsl:with-param name="class" select="'warning'"/>
            <xsl:with-param name="tg-target" select="'tags.html#el-cstatement'"
            />
         </xsl:call-template>
      </xsl:if>
   </xsl:template>
   ...
   <xsl:template name="collection-content-check">
      ...
      <xsl:if test="$context/@article-type = 'collection'">
         <xsl:if
            test="not(count($context/sub-article) + count($context/response) > 1)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">collection content
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="correction-content-check">
      ...
      <xsl:if test="$context/@article-type = 'correction'">
         <xsl:if test="not($context/body) and ($context/front//abstract)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">correction content
                  check</xsl:with-param>
               <xsl:with-param name="description">
      ...
      <xsl:if test="$context/@notes-type = 'disclaimer' and $stream != 'book'">
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     select="'disclaimer notes checking'"/>
                  <xsl:with-param name="description">
                     <xsl:choose>
                        <xsl:when test="$stream = 'manuscript'">
                        ...
                        <xsl:when
                           test="$document-type != 'book' and $document-type != 'book-part'">
   ...
   <xsl:template name="display-object-sec-check">
      ...
      <xsl:if test="$context/@sec-type = 'display-objects'">
         ...
         <xsl:if test="not($context/parent::back) and not($context/parent::sec)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">display-objects sec
                  check</xsl:with-param>
               <xsl:with-param name="description">
         ...
         <xsl:choose>
            ...
            <xsl:when
               test="
                  $context/*[not(self::title)][not(self::fig)
            ...
            <xsl:when
               test="$context/*[not(self::title)][not(@position = 'float')]">
               <xsl:call-template name="make-error">
   ...
   <xsl:template name="disp-quote-content-check">
      <xsl:if test="descendant::preformat">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">disp-quote content
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="doi-format-test">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="not(contains(substring-after($normalized-doi, '10.'), '/'))">
            <xsl:text>false</xsl:text>
         </xsl:when>
   ...
   <xsl:template name="dup-pub-date-check">
      ...
      <xsl:choose>
         <xsl:when
            test="
               $context/@pub-type = 'ppub'
               or $context/@publication-format = 'print' and $context/@date-type = 'pub'">
            <xsl:if
               test="
                  $context/preceding-sibling::pub-date[@pub-type = 'ppub']
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-pubdate'"/>
               </xsl:call-template>
            </xsl:if>
         </xsl:when>
         <xsl:when
            test="
               $context/@pub-type = 'epub'
               or $context/@publication-format = 'electronic' and $context/@date-type = 'pub'">
            <xsl:if
               test="
                  $context/preceding-sibling::pub-date[@pub-type = 'epub']
         ...
         <xsl:when
            test="
               $context/@pub-type = 'collection'
               or $context/@publication-format = 'electronic' and $context/@date-type = 'collection'">
            <xsl:if
               test="
                  $context/preceding-sibling::pub-date[@pub-type = 'collection']
      ...
      <xsl:choose>
         <xsl:when test="$mode = 'strict'">
            <xsl:choose>
               ...
               <xsl:otherwise>
                <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">empty element
                        check</xsl:with-param>
                     <xsl:with-param name="description">
                        <xsl:value-of select="local-name($context)"/>
   ...
   <xsl:template name="epub-date-check">
      ...
      <xsl:if test="$context/day and not($context/month)">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'electronic date check'"/>
            <xsl:with-param name="description">
         ...
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:text>@date-type on [pub-date] must be an accepted value: corrected, preprint, pub, retracted, update. Value provided: </xsl:text>
   ...
   <xsl:template name="ext-link-attribute-check">
      ...
      <xsl:variable name="documented-elt-values"
         select="
            concat(
            'ANZCTR, ',
            'bmrb, ',
      ...
      <xsl:variable name="secret-elt-values"
         select="
            concat(
            'aoi ',
            'arrayexpress ',
      ...
      <xsl:variable name="is-unchecked-elt-type">
         <xsl:call-template name="is-in-list">
            <xsl:with-param name="list"
               select="concat(translate($documented-elt-values, ',', ''), translate($secret-elt-values, ',', ''))"/>
      ...
      <xsl:choose>
         ...
         <xsl:when test="$is-unchecked-elt-type != 1">
            <!--<xsl:message>[[$is-unchecked-elt-type:<xsl:value-of
                  select="$is-unchecked-elt-type"/>]]</xsl:message>-->
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">ext-link check</xsl:with-param>
               <xsl:with-param name="description"
                  select="
   ...
   <xsl:template name="fn-location-check-OBS">
      ...
      <xsl:choose>
         <!-- ignore for books -->
         <xsl:when
            test="$document-type = 'book' or $document-type = 'book-part'"/>
         <!-- Inside front matter -->
   ...
   <xsl:template name="formula-content-test">
      ...
      <xsl:variable name="has-plaintextmath">
         <xsl:if
            test="(normalize-space(translate(text(), ',.?!:;&#x2003;)([]{}', '')) or bold or italic or sup or sub) and not(descendant::xref)"
            >yes</xsl:if>
      </xsl:variable>
      ...
      <xsl:variable name="has-graphic-math">
         <xsl:if
            test="(graphic or inline-graphic) and $has-plaintextmath != 'yes'"
            >yes</xsl:if>
      </xsl:variable>
      ...
      <xsl:choose>
         ...
         <xsl:when test="/article/front/journal-meta/journal-title-group">
            <xsl:choose>
               <xsl:when
                  test="
                     ($has-plaintextmath = 'yes' and $has-mmlmath = 'yes') or
                  ...
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
                        select="'formula content checking'"/>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="fpage-check">
      ...
      <xsl:if
         test="
            not(fpage) and not(elocation-id) and
         ...
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'fpage-elocation check'"/>
            <xsl:with-param name="description">
   ...
   <!-- Template: graphic-alt-version-check 
     ...
     -->
   <!-- *********************************************************** -->
   <xsl:key name="id-to-alternate-form-of" match="*[@alternate-form-of]"
   ...
   <xsl:template name="history-date-type-attribute-check">
      ...
      <xsl:variable name="ok-date-types-article"
         select="
            concat(
            'accepted ',
            'fascicular ',
            ...
            'rev-recd ',
            '')"/>
      <xsl:variable name="ok-date-types-rrn"
         select="
            concat(
            'created ',
            'accepted ',
            ...
            'rev-recd ',
            '')"/>
      <xsl:variable name="ok-date-types-book"
         select="
            concat(
            ...
            'reviewed ',
            '')"/>
      <xsl:variable name="ok-date-types-other"
         select="
            concat(
            $ok-date-types-article, ' ',
            'created ',
      ...
      <xsl:choose>
         <xsl:when test="$stream = 'rrn'">
            ...
            <xsl:if
               test="
                  $context[not(@date-type)] or
                  $date-type-ok-rrn != 1">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"> rrn date check</xsl:with-param>
                  <xsl:with-param name="description"
                     select="
                        concat(
         ...
         <xsl:when test="$stream = 'book'">
            ...
            <xsl:if
               test="
                  ...
                  $date-type-ok-book != 1">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">book date check</xsl:with-param>
                  <xsl:with-param name="description"
                     select="
                        concat(
                        'date inside history must have date-type attribute ',
         ...
         <xsl:otherwise>
            ...
            <xsl:if
               ...
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">article date check</xsl:with-param>
                  <xsl:with-param name="description"
                     select="
                        concat(
                        'date inside history should have date-type attribute ',
   ...
   <xsl:template name="href-content-check">
      ...
      <xsl:if
         test="$context/@xlink:href and normalize-space($context/@xlink:href) = ''">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="href-ext-check">
      ...
      <xsl:if
         test="
            not(contains($href, '.'))
            or (contains($href, '.') and string-length($substr-after-last-dot) > 15)
            or (contains($href, '.') and (contains($substr-after-last-dot, '-')))
            or $substr-after-last-dot = ''">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'@xlink:href checking'"/>
            <xsl:with-param name="description">
               <xsl:text>@xlink:href requires a file extension when used on &lt;</xsl:text>
   ...
   <xsl:template name="is-day">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="
               string-length(normalize-space($input)) = 0
   ...
   <xsl:template name="is-month">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="$accept-name = 'true' and string(number($input)) = 'NaN'">
         ...
         <xsl:otherwise>
            ...
            <xsl:variable name="is-long-name-abbreviated-range">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string"
                     select="'JAN-FEB-MARCH-APRIL-MAY-JUNE-JULY-AUG-SEP-SEPT-OCT-NOV-DEC-'"
                  />
               </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="is-named-season-range-with-fall">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string"
                     select="'SPRING-SUMMER-FALL-WINTER-SPRING-'"/>
               </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="is-named-season-range-with-autumn">
               <xsl:call-template name="test-month-range">
                  <xsl:with-param name="range" select="$normalized-input-case"/>
                  <xsl:with-param name="comparison-string"
                     select="'SPRING-SUMMER-AUTUMN-WINTER-SPRING-'"/>
               </xsl:call-template>
            </xsl:variable>
   ...
   <xsl:template name="issn-problem">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="
               string(number(substring($issn, 9, 1))) = 'NaN'
   ...
   <xsl:template name="issn-pub-type-check">
      <xsl:if
         test="@pub-type and not(@pub-type = 'ppub') and not(@pub-type = 'epub')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
   ...
   <xsl:template name="journal-id-check">
      ...
      <xsl:if test="$context[not(normalize-space())]">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               >journal-meta-check</xsl:with-param>
            <xsl:with-param name="description">
      ...
      <xsl:variable name="ok-journal-id-type-values"
         select="
            concat(
            'archive ',
            'aggregator ',
   ...
   <xsl:template name="journal-meta-check">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:if
               test="not(issn[normalize-space()]) and not(issn-l[normalize-space()])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     >journal-meta-check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="kwd-group-check">
      <xsl:if test="count(kwd) = 1 and count(*) = 1">
         <xsl:if test="contains(kwd, ',') or contains(kwd, ';')">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type"
                  >kwd-group-check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="license-ext-link-content-check">
      ...
      <xsl:choose>
         ...
         <xsl:when
            test="not(contains($str, 'http')) and not(contains($str, 'www'))">
            <!-- content doesn't appear to be URI, we're good -->
         </xsl:when>
         <xsl:when
            test="(contains($str, 'http') or contains($str, 'www')) and (string-length($str) - string-length(translate($str, ' ', '')) > 1)">
            <!-- content contains URI and multiple spaces; likely a sentence -->
         </xsl:when>
         <xsl:when test="starts-with($context/@xlink:href, 'http://')">
            <xsl:choose>
               <xsl:when
                  test="$str = substring-after($context/@xlink:href, 'http://')">
                  <!-- attribute and content almost match, we're good -->
               </xsl:when>
               <xsl:when
                  test="$str = substring-after($context/@xlink:href, 'http://www.')">
                  <!-- attribute and content almost match, we're good -->
               </xsl:when>
               <xsl:when
                  test="substring($str, string-length($str), 1) = '/' and $context/@xlink:href = substring($str, 1, string-length($str) - 1)">
                  <!-- attribute and content almost match, we're good -->
               </xsl:when>
               <xsl:when
                  test="substring(@xlink:href, string-length(@xlink:href), 1) = '/' and $str = substring(@xlink:href, 1, string-length(@xlink:href) - 1)">
                  <!-- attribute and content almost match, we're good -->
               </xsl:when>
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">license/ext-link
                        check</xsl:with-param>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="license-integrity-check">
      <xsl:param name="context"/>
      <xsl:if
         test="$context/@xlink:href[contains(., 'creativecommons.org/licenses')]">
         <xsl:choose>
            <xsl:when
               test="$context//ext-link[contains(@xlink:href, 'creativecommons.org/licenses')]">
               <xsl:variable name="str"
                  select="$context//ext-link/@xlink:href[contains(., 'creativecommons.org/licenses')]"/>
               <xsl:choose>
                  <xsl:when test="$context/@xlink:href = $str">
                     <!-- ext-link points to same uri as license, we're okay -->
                  </xsl:when>
                  <xsl:when
                     test="translate($context/@xlink:href, '/', '') = translate($str, '/', '')">
                  ...
                  <xsl:when test="starts-with($context/@xlink:href, 'www.')">
                     <xsl:choose>
                        <xsl:when
                           test="$str = substring-after($context/@xlink:href, 'www.')">
                           <!-- attribute and content almost match, we're good -->
                        </xsl:when>
                        <xsl:when
                           test="
                              starts-with($str, 'http://') and
         ...
         <xsl:choose>
            <xsl:when
               ...
               <xsl:variable name="nodes"
                  select="$context//ext-link[contains(@xlink:href, 'creativecommons.org/licenses')]"/>
      ...
      <xsl:if
         test="contains(normalize-space($context), 'Creative Commons') and not(contains($context//@xlink:href, 'creativecommons')) and not($context/ali:license_ref[contains(., 'creativecommons')])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">cc no uri license check</xsl:with-param>
            <xsl:with-param name="description">
               <xsl:text>License statement indicates a Creative Commons license, but contains no license URI.</xsl:text>
            </xsl:with-param>
            <xsl:with-param name="class" select="'warning'"/>
   ...
   <xsl:template name="license-ext-link-recursion">
      ...
      <xsl:variable name="one"
         select="substring-after($nodes[1]/@xlink:href, 'licenses')"/>
      <xsl:variable name="two"
         select="substring-after($nodes[2]/@xlink:href, 'licenses')"/>
      <xsl:variable name="onestr">
         <xsl:choose>
            <xsl:when test="contains($one, 'legalcode')">
               <xsl:value-of
                  select="translate(substring-before($one, 'legalcode'), '/', '')"
               />
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:variable name="twostr">
         <xsl:choose>
            <xsl:when test="contains($two, 'legalcode')">
               <xsl:value-of
                  select="translate(substring-before($two, 'legalcode'), '/', '')"
               />
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:if test="$nodes[2]">
         <xsl:choose>
            <xsl:when test="$twostr != $onestr">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">Creative Commons license
                     check</xsl:with-param>
                  <xsl:with-param name="description">License element must not
                     contain different Creative Common license URIs.
                        <xsl:value-of
                        select="substring-after($nodes[1]/@xlink:href, 'licenses')"
                     /> != <xsl:value-of
                        select="substring-after($nodes[2]/@xlink:href, 'licenses')"/>
   ...
   <xsl:template name="list-q-and-a-check">
      ...
      <xsl:if test="$context[@list-type = 'QandA']">
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">q and answer list
                     check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="list-type-check">
      ...
      <xsl:variable name="ok-list-types"
         select="
            concat(
            'order ',
            'bullet ',
	...
	<!-- *********************************************************** -->
	<!-- Template: list-continued-check
        
        The continued-from attribute may not point to the parent list.
     -->
	<!-- *********************************************************** -->
	<xsl:template name="list-continued-check">
		<xsl:param name="context" select="."/>
		<xsl:variable name="lc" select="normalize-space($context/@continued-from)"/>
		<xsl:message>
			<xsl:value-of select="$lc"/>
		</xsl:message>
		<xsl:choose>
			<xsl:when test="$lc = ''"/>
			<xsl:when test="normalize-space($context/@id) = $lc">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'continued list check'"/>
					<xsl:with-param name="description">
						<xsl:text>The continued-from attribute must not reference its parent list.</xsl:text>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
		</xsl:choose>
         ...
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:text>fpage found with no corresponding lpage. lpage is required</xsl:text>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:otherwise>
         </xsl:choose>
   ...
   <xsl:template name="mathml-depricate-atts-chk">
      ...
      <xsl:choose>
         <xsl:when test="$style = 'manuscript'">
            <xsl:if
               test="
                  (contains(translate(@fontsize, '0123456789', '9999999999'), '9') or @fontsize = 'inherited')
               ...
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     select="'mathml deprecated attribute(s)'"/>
                  <xsl:with-param name="description">
                     <xsl:text>Use of deprecated mml:math attribute fontsize, fontweight, fontstyle, fontfamily, or color</xsl:text>
                  </xsl:with-param>
                  <xsl:with-param name="class" select="'error'"/>
                  <xsl:with-param name="tglink"
                     select="'http://www.ncbi.nlm.nih.gov/pmc/pmcdoc/tagging-guidelines/manuscript/tags.html#el-math'"
                  />
               </xsl:call-template>
            </xsl:if>
         </xsl:when>
         <xsl:when test="$style = 'article'">
            <xsl:if
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="class" select="'warning'"/>
                  <xsl:with-param name="tglink"
                     select="'http://www.ncbi.nlm.nih.gov/pmc/pmcdoc/tagging-guidelines/article/tags.html#el-math'"
                  />
               </xsl:call-template>
            </xsl:if>
         </xsl:when>
   ...
   <xsl:template name="mathml-nihms-specific">
      ...
      <xsl:if
         ...
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml deprecated element(s)'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mathml-repeated-element-check">
      ...
      <xsl:variable name="fs1mv">
         <xsl:choose>
            <xsl:when
               test="normalize-space(following-sibling::node()[1]/@mathvariant)">
               <xsl:value-of
                  select="normalize-space(following-sibling::node()[1]/@mathvariant)"
               />
            </xsl:when>
            <xsl:otherwise>
               <xsl:text>none</xsl:text>
      ...
      <xsl:variable name="fs2mv">
         <xsl:choose>
            <xsl:when
               test="normalize-space(following-sibling::node()[2]/@mathvariant)">
               <xsl:value-of
                  select="normalize-space(following-sibling::node()[2]/@mathvariant)"
               />
            </xsl:when>
            <xsl:otherwise>
               <xsl:text>none</xsl:text>
...
<!-- only mml:mtext and mml:mn are sent to this test -->

      <xsl:choose>
         <!-- special processing for mn -->
         <xsl:when
            test="name($context) = 'mml:mn' and name(following-sibling::node()[1]) = '' and name($context) = name(following-sibling::node()[2])">
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
                        select="'mml:mn repeat check'"/>
                     <xsl:with-param name="class" select="$report-level"/>
                     <xsl:with-param name="description">
                        <xsl:value-of select="name($context)"/>
                        <xsl:text> should not follow itself. Numbers must be in a structure or be separated by operators. Complete numbers should be tagged together. Do no separate digits into individual mml:mn tags.</xsl:text>
                     </xsl:with-param>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
         ...
         <xsl:when
            test="name($context) = name(following-sibling::node()[1]) and name($context) = 'mml:mn'">
            <xsl:choose>
         ...
         <!-- special processing for mtext -->
         <xsl:when
            test="name($context) = 'mml:mtext' and name(following-sibling::node()[1]) = '' and name($context) = name(following-sibling::node()[2])">
            <xsl:choose>
               <xsl:when test="$mv != $fs2mv"/>
               <xsl:when
                  test="
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
                        select="'mml:mtext repeat check'"/>
                     <xsl:with-param name="class" select="'warning'"/>
                     <xsl:with-param name="description">
                        <xsl:value-of select="name($context)"/>
                        <xsl:text> should not follow itself. It should include the complete word or phrase. Space (U+0020) characters are preserved in mml:mtext.</xsl:text>
                     </xsl:with-param>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
         ...
         <xsl:when
            test="name($context) = name(following-sibling::node()[1]) and name($context) = 'mml:mtext'">
            <xsl:choose>
               <xsl:when test="$mv != $fs1mv"/>
               <xsl:when
                  test="
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     ...
                     <xsl:with-param name="description">
                        <xsl:value-of select="name($context)"/>
               <xsl:text> should not follow itself. It should include the complete word or phrase. Space (U+0020) characters are preserved in mml:mtext.</xsl:text>                     </xsl:with-param>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
         </xsl:when>
   ...
   <xsl:template name="mathml-top-level-el-check">
      <xsl:if test="*[local-name() = 'prescripts' or local-name() = 'none']">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml top level element check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mathml-mrow-content-check">
      ...
      <xsl:if test="count(*) = 1">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml mrow element check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mathml-parent-el-check">
      ...
      <xsl:if test="not(parent::*[local-name() = $expected-parent])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml parent element check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mathml-subsup-fence-check">
      <!-- Report an error -->
      <xsl:if
         test="translate(normalize-space(*[1]), ']})&#x03009;&#x0232A;&#x027E9;', ']]]]]]') = ']'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml closing fence nesting check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="mml-child-count-check">
      ...
      <xsl:if test="count(./*) != $child-count">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml element child count check'"/>
            <xsl:with-param name="description">
               <xsl:text>mathml element </xsl:text>
               <xsl:value-of select="$name"/>
   ...
   <xsl:template name="mml-attr-value-check">
            ...
            <xsl:when
               ...
               <xsl:variable name="mult-values-valid">
                  <xsl:call-template name="iterate-pipe-str">
                     <xsl:with-param name="test-str"
                        select="translate(normalize-space($attr-value), ' ', '|')"/>
                     <xsl:with-param name="accepted-values"
                        select="$attr-enumerated-values"/>
                  </xsl:call-template>
               </xsl:variable>
      ...
      <xsl:if test="$attr-value-acceptable = 'no'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml attribute value check'"/>
            <xsl:with-param name="description">
               <xsl:text>mathml element </xsl:text>
   ...
   <xsl:template name="iterate-pipe-str">
      ...
      <xsl:variable name="first-value">
         <xsl:value-of
            select="substring-before(substring-after($test-str, '|'), '|')"/>
      </xsl:variable>
      <!--   <xsl:message>
      <xsl:text>&#xA;#########DEBUG#########&#xA;</xsl:text>
      ...
      <xsl:choose>
         ...
         <xsl:when test="contains($accepted-values, $first-value)">
            ...
            <xsl:call-template name="iterate-pipe-str">
               <xsl:with-param name="test-str"
                  select="substring-after($test-str, concat('|', $first-value))"/>
   ...
   <xsl:template name="mathml-desc-text-check">
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">empty math
                  check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:value-of select="local-name(.)"/>
   ...
   <xsl:template name="ms-whitespace-check">
      ...
      <xsl:if test="$content != $clean-content">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'manuscript whitespace check'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="name-alternatives-content-check">
      ...
      <xsl:if test="count($context/name[not(@content-type = 'index')]) > 1">
         <xsl:if
            test="
               not($context/name[not(@content-type = 'index')][@xml:lang])
            ...
            <xsl:call-template name="make-error">
               ...
               <xsl:with-param name="tg-target"
                  select="'tags.html#el-name-alternatives'"/>
            </xsl:call-template>
         </xsl:if>
         ...
         <xsl:if test="$context/name[@xml:lang]">
            <xsl:call-template name="alternatives-xml-lang-check">
               ...
               <xsl:with-param name="nodes"
                  select="$context/name[not(@content-type = 'index')]"/>
            </xsl:call-template>
         </xsl:if>
   ...
   <xsl:template name="name-content-check">
      <xsl:param name="context"/>
      <xsl:if
         test="not($context/surname) and not($context/@name-style = 'given-only')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">
   ...
   <xsl:template name="normalize-to-alphanumeric">
      ...
      <xsl:choose>
         ...
         <xsl:when test="contains($alnumLat1, substring($input, 1, 1))">
         ...
         <xsl:when test="contains($string, '&#xD;')">
            ...
            <xsl:choose>
               <xsl:when
                  ...
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
                        select="'preformat line length check'"/>
                     <xsl:with-param name="description">
   ...
   <xsl:template name="product-to-article-type-check">
      ...
      <xsl:if test="$context/front/article-meta/product">
         <xsl:choose>
            ...
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">article-type
                     attribute-check</xsl:with-param>
                  <xsl:with-param name="description">
   ...
   <xsl:template name="pub-date-check">
      <xsl:choose>
         <xsl:when
            test="pub-date[@pub-type = 'collection'] or pub-date[@date-type = 'collection'][@publication-format = 'electronic']">
            <xsl:choose>
               <xsl:when
                  test="pub-date[@pub-type = 'epub']/day and pub-date[@pub-type = 'epub']/month">
                  <!-- do nothing -->
               </xsl:when>
               <xsl:when
                  test="pub-date[@pub-type = 'epub']/processing-instruction('pmc-write-epub-date')">
                  <!-- do nothing -->
               </xsl:when>
               <xsl:when
                  test="not(pub-date[@pub-type = 'epub']) and not(pub-date[@date-type = 'pub'][@publication-format = 'electronic'])">
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">pub-date
                        check</xsl:with-param>
                     <xsl:with-param name="description">
               ...
               <xsl:when
                  test="
                     (not(pub-date[@pub-type = 'epub']/day) or not(pub-date[@pub-type = 'epub']/month))
         ...
         <xsl:when
            test="pub-date[@pub-type] and (pub-date[@pub-type != 'nihms-submitted' and @pub-type != 'pmc-release'])"/>
      ...
      <xsl:choose>
         <xsl:when test="$context/@pub-type">
            <xsl:if test="$context/@pub-type = 'ppub'">
               <xsl:if
                  test="
                     $context/following-sibling::pub-date[@pub-type = 'collection']
            ...
            <xsl:if test="$context/@pub-type = 'collection'">
               <xsl:if
                  test="
                     $context/following-sibling::pub-date[@pub-type = 'ppub']
         ...
         <xsl:when test="$context/@publication-format">
            <xsl:if
               test="$context/@publication-format = 'print' and $context/@date-type = 'pub'">
               <xsl:if
                  test="
                     $context/following-sibling::pub-date[@pub-type = 'collection']
                     or $context/following-sibling::pub-date[@publication-format = 'collection'][@date-type = 'pub']">
            ...
            <xsl:if
               test="$context/@publication-format = 'collection' and $context/@date-type = 'pub'">
               <xsl:if
                  test="
                     $context/following-sibling::pub-date[@pub-type = 'ppub']
                     or $context/following-sibling::pub-date[@publication-format = 'print'][@date-type = 'pub']">
   ...
   <xsl:template name="pub-id-type-check">
      ...
      <xsl:variable name="ok-names">
         <xsl:choose>
            ...
            <xsl:when test="$stream = 'rrn'">
               <xsl:value-of
                  select="'knolid other publisher-id doi art-access-id pmid'"/>
            </xsl:when>
            <xsl:otherwise>
   ...
   <xsl:template name="pub-type-check">
      ...
      <xsl:variable name="ok-pub-types"
         select="
            concat(
            'epub ',
            'ppub ',
            ...
            <xsl:if
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:choose>
                        <xsl:when
                           test="$context[self::pub-date][@pub-type = 'collection']">
                           <xsl:value-of
                              select="
                                 concat('pub-type attribute can be set to [collection] only ',
                        ...
                        <xsl:otherwise>
                           <xsl:value-of
                              select="
                                 concat('publication-format attribute can be set to [collection] only ',
                                 'when article has another pub-date element with ',
   ...
   <xsl:template name="publication-format-check">
      ...
      <xsl:if test="$str">
         <xsl:choose>
            <xsl:when
               test="$str = 'electronic' or $str = 'print' or $str = 'electronic-print' or $str = 'collection'">
               <!-- these are okay -->
            </xsl:when>
            <xsl:otherwise>
               <xsl:call-template name="make-error">
   ...
   <xsl:template name="related-article-check">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$context[@ext-link-type = 'pmc']">
            <xsl:if
               test="
                  not($context[@id and @related-article-type and @vol and @page])
                  and not($context[@id and @related-article-type and @vol and @elocation-id])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     select="'related-article check'"/>
                  <xsl:with-param name="description">
                     <xsl:text>The attributes included in related-article are insufficient to identify this element's target article. Please review PMC's tagging guidelines to help you construct a fully-identified related-article element.</xsl:text>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target"
                     select="'tags.html#el-relart'"/>
               </xsl:call-template>
            </xsl:if>
      ...
      <xsl:variable name="ok-list"
         select="
            ...
            'retracted-article ',
            'retraction-forward ',
            'retraction-republication ',
            'retracted-republished-article ',
            'reviewed-article ',
            'update-in ',
            'update-of ',
            'updated-article ',
      ...
      <xsl:variable name="rel-art-value" select="$context/@related-article-type"/>
      ...
      <xsl:variable name="ok-ext-link-values"
         select="
            concat(
            'aoi ',
            'doi ',
   ...
   <xsl:template name="ms-related-article-check">
      ...
      <xsl:if
         test="
            @related-article-type != 'corrected-article' and
            @related-article-type != 'retracted-article' and
            @related-article-type != 'retracted-republished-article' and
            ...
            @related-article-type != 'object-of-concern' and 
            @related-article-type != 'reprinted-article'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">related-article type
               check</xsl:with-param>
            <xsl:with-param name="description">
      ...
      <xsl:if
         test="not(@ext-link-type = 'pmcid') and not(@ext-link-type = 'pmid')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">related-article @ext-link-type
               check</xsl:with-param>
            <xsl:with-param name="description">
      ...
      <xsl:if test="not(number(@xlink:href))">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">related-article @xlink:href
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="related-object-check">
      ...
      <xsl:if
         test="
            (@source-id and not(@source-id-type)) or
      ...
      <xsl:if test="@document-type = 'article'">
         <xsl:choose>
            <xsl:when
               test="@document-id-type = 'doi' or @document-id-type = 'pmcid' or @document-id-type = 'pmid'">
               <!-- these are okay -->
            </xsl:when>
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type"
                     select="'related-object check'"/>
                  <xsl:with-param name="description">&lt;related-object&gt; with
                     document-type="article" must have document-id-type of [doi
                     | pmcid | pmid]. Supplied value is [<xsl:value-of
                        select="@document-id-type"/>].</xsl:with-param>
               </xsl:call-template>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:if>
      ...
      <xsl:if test="@document-type = 'article' and not(@document-id)">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'related-object check'"/>
            <xsl:with-param name="description">&lt;related-object&gt; with
               document-type="article" must have a document-id.</xsl:with-param>
         </xsl:call-template>
      </xsl:if>
      ...
      <xsl:if test="@document-type = 'article'">
         ...
         <!-- Check the related-article-type values -->
         <xsl:variable name="rel-art-value" select="$context/@link-type"/>
         <xsl:if test="$rat-ok != 1">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type" select="'related-object check'"/>
               <xsl:with-param name="description"
   ...
   <xsl:template name="related-article-xlink-extlinktype-check">
      ...
      <xsl:if
         test="$good-doi = 'true' and $eltype != 'doi' and $eltype != 'DOI'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'related-article check'"/>
            <xsl:with-param name="description">
      ...
      <xsl:variable name="page">
         <xsl:choose>
            ...
            <xsl:when test="/article/front/article-meta/fpage[@seq]">
               <xsl:value-of
                  select="concat(/article/front/article-meta/fpage, /article/front/article-meta/fpage/@seq)"
               />
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:choose>
         <xsl:when test="@ext-link-type = 'doi'">
            <xsl:if test="@xlink:href = $doi">
               <xsl:choose>
                  <xsl:when
                     test="@related-article-type = 'peer-reviewed-article' or 
                     @related-article-type = 'reviewed-article' or 
                     @related-article-type = 'editor-report' or 
                     @related-article-type = 'referee-report' "/>
                  <xsl:when test="$arttype = 'letter' or $arttype = 'reply' or @related-article-type = 'alt-language'">
                     <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type" select="'related-article letter reply check'"/>
                        <xsl:with-param name="class" select="'warning'"/>
                        <xsl:with-param name="description">
            ...
            <xsl:if test="@page = $page and @vol = $vol">
               <xsl:choose>
                  <xsl:when test="$arttype = 'letter' or $arttype = 'reply' or @related-article-type = 'alt-language'">
                     <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type"
                           select="'related-article letter reply check'"/>
                        <xsl:with-param name="class" select="'warning'"/>
                        <xsl:with-param name="description">
   ...
   <xsl:template name="related-article-to-article-type-attribute">
      ...
      <xsl:variable name="art-type"
         select="normalize-space($context/ancestor::article/@article-type)"/>
      ...
      <xsl:variable name="msg_start">
         <xsl:value-of
            select="
               concat('Either the ', $context-att-name, ' attribute in ',
      ...
      <xsl:choose>
         ...
         <xsl:when test="$context-type-att = 'addended-article'">
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type"
                        select="concat($context-att-name, ' check')"/>
                     <xsl:with-param name="description"
                        select="
                           concat(
                           $msg_start,
         ...
         <xsl:when test="$context-type-att = 'retracted-article' or $context-type-att='retracted-republished-article'">
            <xsl:choose>
               <!-- This is correct -->
   ...
   <xsl:template name="season-check">
      ...
      <xsl:choose>
         <xsl:when
            test="
               $context/ancestor::citation or $context/ancestor::nlm-citation
         ...
         <xsl:choose>
            ...
            <xsl:when
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <xsl:call-template name="make-error">
                        <xsl:with-param name="error-type">sec-type check</xsl:with-param>
                        <xsl:with-param name="description">
                           <xsl:text>sec should have sec-type attribute set to 'materials|methods'</xsl:text>
                        </xsl:with-param>
                        <xsl:with-param name="tg-target"
                           select="'dobs.html#dob-methods'"/>
                     </xsl:call-template>
                  </xsl:otherwise>
               </xsl:choose>
            </xsl:when>
            ...
            <xsl:when
               ...
               <xsl:choose>
                  <xsl:when
                     test="$context/@sec-type[contains(., 'methods')] or //sec/@sec-type[contains(., 'methods')]">
                     <!-- this is right -->
                  </xsl:when>
   ...
   <xsl:template name="ms-extended-data-sec-test">
      <xsl:if test="@sec-type = 'extended-data'">
         <xsl:choose>
            <xsl:when test="normalize-space(title) = 'Extended Data'"/>
            <!-- good -->
            <xsl:otherwise>
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">section title check</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:if test="not(fig) and not(table-wrap)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">extended data section
                  check</xsl:with-param>
               <xsl:with-param name="description">
         ...
         <xsl:if
            test="fig[@position != 'anchor'] or table-wrap[@position != 'anchor'] or boxed-text[@position != 'anchor']">
            <xsl:call-template name="make-error">

               <xsl:with-param name="error-type">extended data object
                  check</xsl:with-param>
               <xsl:with-param name="description">
   ...
   <xsl:template name="string-conf-element-check">
      <xsl:if test="string-conf">
         <xsl:call-template name="make-error">
            <xsl:with-param name="class" select="'warning'"/>
            <xsl:with-param name="error-type">string-conference
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="string-date-check">
      ...
      <xsl:variable name="number-of-spaces">
         <xsl:call-template name="count-spaces">
            <xsl:with-param name="input"
               select="normalize-space($normalized-date)"/>
         </xsl:call-template>
      </xsl:variable>
      ...
      <xsl:choose>
         ...
         <xsl:when test="$number-of-tokens = 2">
            ...
            <xsl:choose>
               <xsl:when
                  test="$token1-is-month = 'true' and $token2-is-year = 'true'">
                  <xsl:call-template name="make-error">

<xsl:with-param name="error-type">string-date check</xsl:with-param>
                     <xsl:with-param name="description">
...
<xsl:with-param name="error-type">string-date check</xsl:with-param>
   ...
   <xsl:template name="string-name-content-check">
      <xsl:if test="count(surname) > 1 or count(given-names) > 1">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">string-name check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="subj-group-check">
      ...
      <xsl:if
         test="
            $context[@subj-group-type = 'heading'] and
            count($context/subject) > 1">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">subj-group
               check</xsl:with-param>
            <xsl:with-param name="description">
	...
	<!-- *********************************************************** -->
	<!-- Template: table-check 
   
        1) for books: may have only one tbdoy (warning) -->
	<!-- *********************************************************** -->
	<xsl:template name="table-check">
		<xsl:param name="context" select="."/>
		
		<xsl:if test="$stream = 'book' and count($context/tbody) &gt; 1">
			<xsl:call-template name="make-error">
				<xsl:with-param name="class">warning</xsl:with-param>
				<xsl:with-param name="error-type">table check</xsl:with-param>
				<xsl:with-param name="description">
					<xsl:text>A &lt;table&gt; may include only one &lt;tbody&gt;</xsl:text>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
   ...
   <xsl:template name="test-month-range">
      ...
      <xsl:choose>
         ...
         <xsl:when test="not($recursing)">
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="test-month-range">
                     <xsl:with-param name="recursing" select="1"/>
                     <xsl:with-param name="comparison"
                        select="normalize-space(substring-before($normalized-range, '-'))"/>
                     <xsl:with-param name="next-comparison"
                        select="
         ...
         <!-- Found a match -->
         <xsl:when
            test="starts-with($comparison-string, concat($comparison, '-'))">
            <xsl:choose>
   ...
   <xsl:template name="tex-math-content-check">
      ...
      <xsl:if
         test="not(contains(., 'begin{document}')) or not(contains(., 'end{document}'))">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="title-fn-check-OBS">
      ...
      <xsl:if test="$context/fn">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">article-title/subtitle
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="trans-abstract-check">
      ...
      <xsl:if test="$local-lang = $art-lang-att">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">trans-abstract language attribute
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="trans-atts-check">
      ...
      <xsl:if test="not($context/@xml:lang)">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">
               <xsl:value-of
                  select="concat(local-name($context), ' language attribute check')"
               />
            </xsl:with-param>
            <xsl:with-param name="description">
               <xsl:value-of
                  select="concat(local-name($context), ' must include the xml:lang attribute.')"
               />
            </xsl:with-param>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template name="trans-title-check">
      ...
      <xsl:variable name="at-lang">
               <xsl:choose>
                  <xsl:when test="preceding::article-title/@xml:lang">
         			 <xsl:call-template name="knockdown">
           					 <xsl:with-param name="str">
                    			<xsl:value-of select="preceding::article-title/@xml:lang"/>
            						</xsl:with-param>
         				</xsl:call-template>
                  </xsl:when>
                  <xsl:otherwise>
      ...
      <xsl:if test="$local-lang = $art-lang-att and not(ancestor::ref)">
				<xsl:if test="$at-lang = $art-lang-att or $at-lang='none'">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type">trans-title language attribute
               check</xsl:with-param>
            <xsl:with-param name="description">
   ...
   <xsl:template name="trans-title-group-content-check">
      <xsl:for-each select="child::node()">
         <xsl:if
            test="@xml:lang and @xml:lang != parent::trans-title-group/@xml:lang">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">trans-title-group language
                  attribute check</xsl:with-param>
               <xsl:with-param name="description">
               ...
               <xsl:with-param name="tg-target"
                  select="'tags.html#el-transtitlegrp'"/>
            </xsl:call-template>
         </xsl:if>
         <xsl:if
            test="name() = 'trans-subtitle' and not(ancestor::title-group/subtitle) and not(ancestor::journal-title-group/journal-subtitle)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">trans-subtitle
                  check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>&lt;trans-subtitle&gt; must correspond to a &lt;subtitle&gt; in the ancestor &lt;</xsl:text>
                  <xsl:value-of
                     select="name(parent::trans-title-group/parent::node())"/>
                  <xsl:text>&gt;.</xsl:text>
               </xsl:with-param>
               <xsl:with-param name="tg-target"
                  select="'tags.html#el-transtitlegrp'"/>
      ...
      <xsl:if
         ...
         <xsl:choose>
            ...
            <xsl:otherwise>
               <!-- report error -->
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">web links
                     check</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:choose>
            <xsl:when
               test="$context[starts-with(normalize-space(@xlink:href), 'http://')]">
               <!-- No problem-->
            </xsl:when>
   ...
   <xsl:template name="xlink-attribute-check">
      ...
      <xsl:if test="$context/@xlink:*[not(name() = 'xlink:type')]">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'xlink-attribute checking'"/>
            <xsl:with-param name="description">
   ...
   <xsl:template name="xref-check">
      ...
      <xsl:choose>
			<xsl:when test="not(@ref-type) and not(@rid)">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">xref attribute check</xsl:with-param>
              <xsl:with-param name="class">warning</xsl:with-param>
                <xsl:with-param name="description">
                  <xsl:text>xref element must have @ref-type and @rid.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>		
				</xsl:when>
         <xsl:when test="$context[@ref-type and not(@rid)] and $stream != 'rrn'">
            ...
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">xref check</xsl:with-param>
               <xsl:with-param name="description">
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'an app element, app-group element, a sec inside an app, or a sec inside back'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target" select="'tags.html#el-xref'"
         ...
         <xsl:when
            test="
               $context/@ref-type = 'author-notes' or
               $context/@ref-type = 'author-note'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::author-notes
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'an author-notes element or a fn or corresp inside author-notes'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
         ...
         <xsl:when test="$context/@ref-type = 'bibr'">
            <xsl:if
               test="not(id($context/@rid)[self::citation or self::ref or self::note or self::mixed-citation or self::element-citation])">
               <xsl:call-template name="make-error">
                  <xsl:with-param name="error-type">xref
                     check</xsl:with-param>
                  <xsl:with-param name="description">
         ...
         <xsl:when test="$context/@ref-type = 'boxed-text'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::boxed-text])
         ...
         <xsl:when test="$context/@ref-type = 'corresp'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::corresp
                  or self::fn
                  or self::author-notes
                  or parent::boxed-text[@content-type = 'author-notes']])
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'a corresp element, a fn element, or an author-notes element'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
         ...
         <xsl:when test="$context/@ref-type = 'disp-formula'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::disp-formula
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'an element of type disp-formula, disp-formula-group, inline-formula, fig, or supplementary-material'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
         ...
         <xsl:when
            test="$context/@ref-type = 'fig' or $context/@ref-type = 'plate' or $context/@ref-type = 'scheme'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::fig or self::fig-group])
                  ...
                  <xsl:with-param name="error-type">xref
                   ...
                   <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                       <xsl:with-param name="expected-target"
                           select="'a fig or fig-group element'"/>
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:if>
         ...
         <xsl:when test="$context/@ref-type = 'fn'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::fn or self::author-notes])
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'a fn element or an author-notes element'"/>
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:if>
         ...
         <xsl:when test="$context/@ref-type = 'kwd'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::kwd or self::kwd-group])
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'a kwd or kwd-group element'"/>
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:if>
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'an element of type list, list-item, glossary, gloss-group, def-list, or def-item'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'an element of type sec, ack, app, boxed-text, ref-list, or notes'"
                        />
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
         ...
         <xsl:when test="$context/@ref-type = 'statement'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::statement])
         ...
         <xsl:when test="$context/@ref-type = 'supplementary-material'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::supplementary-material
         ...
         <xsl:when test="$context/@ref-type = 'table'">
            <xsl:if
               test="
                  not(id($context/@rid)[self::table-wrap
               ...
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:call-template name="write-xref-error-description">
                        <xsl:with-param name="xref" select="$context"/>
                        <xsl:with-param name="expected-target"
                           select="'a table-wrap or table-wrap-group element'"/>
                     </xsl:call-template>
                  </xsl:with-param>
               </xsl:call-template>
            </xsl:if>
         ...
         <xsl:when test="$context/@ref-type = 'table-fn'">
            <xsl:if
               test="
                  not(id($context/@rid)[(self::fn and ancestor::table-wrap)
                  or self::td])
   ...
   <!-- xref-check -->
	...
	<!-- *********************************************************** -->
	<!-- Template: xref-aff-book-check
   
        Check that the xref points
        to an aff that belongs to the same 
        book-meta or book-part-meta as the xref itself
     -->
	<!-- *********************************************************** -->
	<xsl:template name="xref-aff-book-check">
		<xsl:if test="@ref-type = 'aff'">
			<xsl:variable name="target" select="id(@rid)"/>
			<xsl:if test=" 
				    $target and 
				    generate-id(ancestor::sec-meta | ancestor::book-part-meta | ancestor::book-meta | ancestor::collection-meta) != 
				    generate-id($target/ancestor::sec-meta  | $target/ancestor::book-part-meta | $target/ancestor::book-meta | $target/ancestor::collection-meta)">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type"
						select="'xref aff book check'"/>
					<xsl:with-param name="description">
						<xsl:text>the xref with the @rid value '</xsl:text>
						<xsl:value-of select="@rid"/>
						<xsl:text>' does not point to an aff element within the same book-part-meta, book-meta, or collection-meta.</xsl:text>
					</xsl:with-param>
					<xsl:with-param name="tg-target" select="'tags.html#el-aff'"/>
				</xsl:call-template>
			</xsl:if>			
		</xsl:if>
	</xsl:template>
