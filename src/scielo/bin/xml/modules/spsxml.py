...
class SPSXML(object):
    ...
    def __init__(self, input_pkgfiles, output_pkgfiles):
        self.input_pkgfiles = input_pkgfiles
        self.output_pkgfiles = output_pkgfiles
        self.spsxmlcontent = SPSXMLContent(fs_utils.read_file(self.input_pkgfiles.filename))
    ...
    def doc(self):
        if self.xml is not None:
            a = article.Article(self.xml, self.input_pkgfiles.name)
            a.new_prefix = self.input_pkgfiles.name
            return a

    def pack(self):
        self.input_pkgfiles.copy(self.output_pkgfiles.path)
        fs_utils.write_file(self.output_pkgfiles.filename, self.spsxmlcontent.content)
