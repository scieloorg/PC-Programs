...
class ArticleValidator(object):
    ...
    def __init__(self, xml_structure_validator, xml_content_validator):
        self.xml_structure_validator = xml_structure_validator
        self.xml_content_validator = xml_content_validator

    def validate(self, article, outputs, pkg_issue_validations):
        artval = ArticleValidations(pkg_issue_validations)
        artval.validations['xmlstructure'].message = xml_structure_validator.validate(outputs)
        artval.validations['xmlcontent'].message, artval.article_display_report = self.xml_content_validator.validate(article, outputs)
        if self.xml_content_validator.is_xml_generation:
            stats = artval.validations['xmlcontent'].statistics_display(False)
            title = [_('Data Quality Control'), article.new_prefix]
            fs_utils.write_file(outputs.data_report_filename, html_reports.html(title, stats + artval.validations['xmlcontent'].message))
        return artval


class ArticleValidations(object):

    def __init__(self, issue_data_validations):
