...
SET PATH_IMG=%ISSUE_PATH%\pmc\pmc_img
SET WORK_ART=%WORK%\%FILENAME%

set PDF_PATH=%ISSUE_PATH%\pdf
...
if exist %this2that% %XML_TOOLS_PATH%\..\cfg\mx seq=%this2that% lw=9999 "proc='d*a9901{%PACK%{a9902{%WORK_ART%{a9903{%PATH_PMC_IMG%{a9904{%PATH_IMG%{a9905{%XML_TOOLS_PATH%{a9906{%PATH_PMC_IMG%{"

"pft=if p(v1) then 
         if p(v2) then 
             'call %XML_TOOLS_PATH%\rename_img.bat ',v1,' ',v2,' ',v9901,' ',v9902,' ',v9903,' ',v9904,' ',v9905,/ 
         else 
             'call %XML_TOOLS_PATH%\rename_article.bat %FILENAME% ',v1,' %ISSUE_PATH% %XML_FILE% %PACK% %WORK_ART% %PDF_PATH%',
             
             if '%PDF_PATH%'<>'%ISSUE_PATH%\pdf' then 
                '%FILENAME%.pdf'/ 
             else 
...
%XML_TOOLS_PATH%\..\cfg\mx "seq=%temp_batch_seq%" lw=9999 "pft=if not v1:'%temp_batch_partial%' then v1/ fi" now | sort >  %temp_batch_complete%
