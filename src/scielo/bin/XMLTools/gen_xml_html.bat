...
if exist %temp_file%    del /F %temp_file%
if exist %temp_sh_file% del /F %temp_sh_file%
...
if exist %temp_file% %XML_TOOLS_PATH%\..\cfg\mx seq=%temp_file% lw=9999 "pft=if s(mpu,v1,mpl):'ERROR' then 'copy %temp_file% %temp_errfile%'/ fi" now > %temp_sh_file%
...
echo %JAVA_EXE% -cp %XML_TOOLS_PATH%\core\XMLCheck.jar br.bireme.XMLCheck.XMLCheck %OUTPUT_XML% --validate >> %log_file%
...
echo %JAVA_EXE% -jar %XML_TOOLS_PATH%\core\saxon8.jar -novw -w0 -o %temp_file% %OUTPUT_XML% %XSL_HTML% >> %log_file%
