...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		...
		<xsl:choose>
			<xsl:when test="//scielo-url=''">
				<ul>
					<xsl:if test="//scielo-url=''">
						<li>
							<p>
								<b>Error: Missing url. e.g.: http://www.scielo.br</b>
								<br/> Solution: Edit the file
								/xml_scielo/config/PubMed/journals/journals.seq, with the following
								format:<br/> journal_abbrev start_year_that_make_part_of_the_query
									<b>url</b>
								<br/>
							</p>
							<!--p>
	...
	<xsl:template match="publishing_dateiso" mode="listData">
		<xsl:value-of select="substring(occ, 1, 4)"/>:<xsl:value-of select="substring(occ, 5, 4)"
		/>:2010 [dp]</xsl:template>
