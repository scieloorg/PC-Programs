...
%WXIS% IsisScript=xis\xml_scielo_getXSL.xis acron=%acron% issueid=%issueid% generate_linkout=%generate_linkout% ahpdate=%ahpdate% cipar=%CIPAR_FILE% xslList=temp\xml_scielo_xsl.seq debug=%debug% destination=%DATABASE_DESTINATION% >> %LOG%
more temp\xml_scielo_xsl.seq >> %LOG%

%MX% "seq=temp\xml_scielo_xsl.seq " lw=9999 "pft=if p(v2) and p(v3) then 'call scripts\applyXSL.bat ',v1,' ',v2,' ',v3,' %acron% %issueid% ',v4,| |v5/ fi" now > temp\xml_scielo_transform_and_transfer.bat
