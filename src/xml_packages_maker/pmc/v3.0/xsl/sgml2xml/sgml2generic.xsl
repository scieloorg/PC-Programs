  ...
  <fig id="fg-14">
   ...
   <caption><p>View C: In Motion, A Blur on Feet</p></caption>
	...
	<xsl:template match="back//*[contains(name(),'contrib')]//title">
		...
		<xsl:choose>
			<xsl:when test="../..//node()[contains(name(),'monog')] or ../../..//node()[contains(name(),'vmonog')]">
				<chapter-title>
					<xsl:apply-templates select="*|text()"/>
					<xsl:apply-templates select="../subtitle" mode="title"/>
				</chapter-title>
	...
	<xsl:template match="xref[@rid!='']">
		...
		<xsl:choose>
			...
			</xsl:when><xsl:when test="$xref_id[@id=$rid]">
				<xref>
					<xsl:apply-templates select="@*"/>
			...
			<xsl:otherwise><xsl:comment><xsl:value-of select="$rid"/><xsl:apply-templates select="$xref_id" mode="display-id"/></xsl:comment><xsl:value-of select="."/></xsl:otherwise>
		</xsl:choose>
