...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="version">xml-file</xsl:variable>
	<xsl:variable name="var_IMAGE_PATH">
		<xsl:choose>
			<xsl:when test="$path_img!=''">
				<xsl:value-of select="$path_img"/>
			</xsl:when>
			<xsl:otherwise/>
		</xsl:choose>
	</xsl:variable>

	<xsl:output method="html" omit-xml-declaration="no" encoding="utf-8" indent="no"/>
	...
	<xsl:template name="make-html-header">
		<head>
			...
			<style type="text/css">
				...
				.navigation DIV#issues,
				.navigation DIV#articles,
				.navigation DIV#articlesSearch,
